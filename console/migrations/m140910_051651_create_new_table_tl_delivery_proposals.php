...
/*
 * Этапы создания доставки груза из Алматы в Актюбе
 * [tl_delivery_proposals] руками клиент, оператор или через апи создаем заявку на доставку груза из Алмыты в Актюбе
 *  1.1 [tl_delivery_proposal_orders] Прикрепляет номера накладных приходных и/или расходных.
 *   ? Как будет происходить прикрепление накладных????
 *
 * [tl_delivery_proposals] оператор видит все заявки на доставку сгрупперованные по направлениям и статусам выполнения новые вверху и подсвечиваются. дата по доставки не учитывается
 * [tl_delivery_proposals] Оператор на складе заходит в заявку (пример: Алматы-Актюбе) и строит маршрут.
 *
 *
 *
 *
...
class m140910_051651_create_new_table_tl_delivery_proposals extends Migration
{
    public function up()
    {
        $tableOptions = null;
        ...
        $this->createTable('{{%tl_delivery_proposals}}', [
            'id' => Schema::TYPE_PK,
    ...
    {
        $this->dropTable('{{%tl_delivery_proposals}}');
    }
}