...
namespace common\clientObject\erenRetail\inbound\service;

use common\clientObject\erenRetail\inbound\repository\InboundRepository;
...
class InboundOrderUploadService
{
    ...
    private function makeDTOForCreateInboundOrder($dto)
    {
        ...
        $dtoForCreateInboundOrder->comment = $dto->comment;
		$dtoForCreateInboundOrder->order_type = $dto->order_type;
        ...
        if (is_file($dto->pathToOrderFile) && file_exists($dto->pathToOrderFile)) {
            $preparedDataFromFile = $dto->order;  //SpreadsheetService::parseFile($dto->pathToOrderFile);
