...
//			],
    ...
    {
        return [
            [['client_id', 'product_id', 'barcode'], 'required'],
            [['client_id', 'product_id', 'created_user_id', 'updated_user_id', 'created_at', 'updated_at'], 'integer'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'updated_user_id' => Yii::t('titles', 'Modified User Id'),
    ...
    {
        return (int)self::find()->andWhere('client_id = :client_id AND barcode = :barcode',[':client_id'=>$client_id,':barcode'=>$barcode])->count();
    }
    ...
    {
        $product = null;
        if($pb = self::find()->andWhere('client_id = :client_id AND barcode = :barcode',[':client_id'=>$client_id,':barcode'=>$barcode])->one()) {
            $product = $pb->product;
        }
        return $product;
    }
