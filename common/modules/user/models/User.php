...
//    const USER_REGISTER_DONE = 'user_register_done';
    ...
    * */
//    const USER_TYPE_UNDEFINED = 0; // Тип не указан
    const USER_TYPE_CLIENT = 1; //Клиент + Используем
//    const USER_TYPE_STORE_MANAGER = 2; //Менеджер клиента
    const USER_TYPE_STOCK_WORKER = 2; //Работник склада + Используем
    const USER_TYPE_AGENT_EMPLOYEE = 3; //Работник транспортного агента
    const USER_TYPE_EXTERNAL_CLIENT = 4; //Работник транспортного агента
    const USER_TYPE_BOSS = 999; //БОСС + Используем
    const USER_TYPE_POINT_WORKER = 5; // Сотрудник точки + Используем
    const USER_TYPE_POINT_CLIENT_WORKER = 6; // Клиент созданные на стороне точки, чем он отличается от клиента
//    const USER_TYPE_CLIENT_LOGIST = 5; //Логист клиент
...
//    }
    ...
    public function scenarios()
    {
        return [
            ...
            'create'   => ['username', 'email', 'password','user_type', 'role'],
            'create_external_client'   => ['username', 'email', 'password','user_type', 'role'],
            'create_client_from_order'   => ['username','password','user_type'],
            'update_external_client'   => ['email', 'password','user_type', 'role'],
            'update'   => ['username', 'email', 'password','user_type', 'role'],
            'settings' => ['username', 'email', 'password','user_type']
        ];
    }
    ...
    public function rules()
    {
        return [
            ...
            // User Type: Client, Operator, Owner shop
            ['user_type', 'trim'],
            ['user_type', 'required', 'on' => [ 'create', 'update']],
        ];
    }
...
//    }
    ...
    /*
     * USER_TYPE_UNDEFINED = 0; // Тип не указан
     * USER_TYPE_CLIENT = 1; //Клиент
     * USER_TYPE_STORE_MANAGER = 2; //Менеджер клиента
     * USER_TYPE_STOCK_WORKER = 3; //Работкин склада
     * USER_TYPE_BOSS = 4; //БОСС
     * */
    public static function getUserTypeArray($key = null)
    {
        $data = [
//            self::USER_TYPE_UNDEFINED => Yii::t('forms', 'Undefined'), //Тип не указан
            self::USER_TYPE_CLIENT => Yii::t('titles', 'Клиент'), //Клиент
//            self::USER_TYPE_STORE_MANAGER => Yii::t('titles', 'Менеджер клиента'), //Менеджер клиента
            self::USER_TYPE_STOCK_WORKER => Yii::t('titles', 'Работник склада'), //Работник склада
            self::USER_TYPE_BOSS => Yii::t('titles', 'БОСС'), //БОСС
        ];
    ...
    {
        $data = self::getUserTypeArray();
        return isset($data[$this->user_type]) ? $data[$this->user_type] : Yii::t('titles','-Не указан-');
    }

    /* TODO:add banned and deleted user status display
    * Get User title by user_id
    *
    * */
    public function getUserTitle($user_id = null)
    {
        if (!is_null($user_id)) {
