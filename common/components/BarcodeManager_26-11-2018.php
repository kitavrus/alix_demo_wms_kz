...
//        return false;
    ...
    public static function isReturnBoxBarcode($barcode) {
        ...
        if(!BarcodeManager::isDefactoBox($barcode) && !BarcodeManager::isBoxOnlyOur($barcode)) {
            return false;
        }

        // Если передали наш шк короба
        if(BarcodeManager::isBoxOnlyOur($barcode)) {
            $barcode = Stock::find()->select('inbound_client_box')->andWhere(['primary_address'=>$barcode])->scalar();
        }

        $isExistInInbound = InboundOrder::find()->andWhere(['order_number'=>$barcode])->exists();
        $isExistInReturnItem = ReturnOrderItems::find()->andWhere(['client_box_barcode'=>$barcode])->exists();
