...
class BarcodeManager extends Component
{
	...
	const OUTBOUND_BOX_BARCODE_PREFIX = '4000';
	const OUTBOUND_BOX_BARCODE_LENGTH = 12;


	public static function isValidInboundBoxBarcode($boxBarcode)
	{
		// Проверка на то, что строка начинается с "5000"
		if (substr($boxBarcode, 0, 4) != self::INBOUND_BOX_BARCODE_PREFIX) {
			return 'ШК короба должен начинаться с 5000';
		}

		// Проверка длины
		if (strlen($boxBarcode) != self::INBOUND_BOX_BARCODE_LENGTH) {
			return 'ШК короба должен быть ровно 12 символов длиной';
		}

		// Проверка на то, что строка состоит только из цифр
		if (!ctype_digit($boxBarcode)) {
			return 'ШК короба должен содержать только цифры';
		}

		return null;
	}

	public static function isValidOutboundBoxBarcode($boxBarcode)
	{
		// Проверка на то, что строка начинается с "4000"
		if (substr($boxBarcode, 0, 4) != self::OUTBOUND_BOX_BARCODE_PREFIX) {
			return 'ШК короба должен начинаться с 4000';
		}

		// Проверка длины
		if (strlen($boxBarcode) != self::OUTBOUND_BOX_BARCODE_LENGTH) {
			return 'ШК короба должен быть ровно 12 символов длиной';
		}
    ...
    * */
    ...
    public static function isDefactoProduct($product)
    {
        ...
        if(in_array($code, ["230",'900','868','233'])) {
            return true;
        }
        return false;
    }
...
//        return ProductBarcodes::find()->where(['barcode'=>$barcode])->exists();
         return Stock::find()->andWhere(['product_barcode'=>$barcode])->exists() OR OutboundOrderItem::find()->andWhere(['product_barcode'=>$barcode])->exists();
    }
...
//        return false;
    ...
    public static function isBoxLotOrReturnBox($boxOrProductBarcode,$addressList = [],$boxBarcode = '') {
        return self::_isProductBoxTypeQuery($boxOrProductBarcode,[Stock::IS_PRODUCT_TYPE_RETURN,Stock::IS_PRODUCT_TYPE_LOT_BOX],$addressList,$boxBarcode);
    }
    ...
    private static function _isProductBoxTypeQuery($boxOrProductBarcode,$productType,$addressList = [],$boxBarcode = '') {
        return Stock::find()
...
//        )->exists();
    ...
    public static function isReturnBoxBarcode($boxBarcode) {
        ...
        if(BarcodeManager::isBoxOnlyOur($boxBarcode)) {
            $boxBarcode = Stock::find()->select('inbound_client_box')->andWhere(['primary_address'=>$boxBarcode])->scalar();
        }
    ...
    public static function isReturnBoxBarcodeInventory($boxBarcode) {
        ...
        // Если передали наш шк короба
        if(BarcodeManager::isBoxOnlyOur($boxBarcode)) {
            $boxBarcode = Stock::find()->select('inbound_client_box')->andWhere(['inventory_primary_address'=>$boxBarcode])->scalar();
        }
    ...
    public static function isOneBoxOneProductInventory($boxBarcode, $clientID = null, $warehouseID = null)
    {
        //return self::isBoxTypeLotBox($boxBarcode);

        if(BarcodeManager::isBoxOnlyOur($boxBarcode)) {
            return Stock::find()->andWhere(['inventory_primary_address'=>$boxBarcode])
                ->andFilterWhere(['client_id'=>$clientID])
                ->andFilterWhere(['warehouse_id'=>$warehouseID])
                ->count() == 1;
        }
    ...
    public static function mapM3ToBoxSize($key)
    {
        $map = [
            ...
            '8'=> '52x47x10/8',
            '9'=> '55x38x13/9',
            '10'=>'40x40x20/10',// TODO неверный размер!!!!
            '11'=>'40x40x20/11',
            '12'=>'42x30x28/12',
            ...
            '33'=>'76x62x21/33', // 76x66x20/33
            '34'=>'73x60x23/34', // 94x64x17/34 0.102
            '35'=>'76x58x24/35', // 0.105 '35'=>'73x52x28/35',
            '36'=>'75x55x26/36',
            '37'=>'78x60x24/37', // 72x55x28/37  '37'=>'60x46x40/37',
            '38'=>'60x45x42/38',
            '39'=>'82x55x26/39',
            '40'=>'60x40x50/40',
            '41'=>'60x45x46/41',
            '42'=>'78x65x25/42', // 72x50x35/42
            '43'=>'85x58x26/43',
            '44'=>'82x62x26/44',
            '45'=>'72x54x35/45', // '45'=>'72x53x35/45',
            '46'=>'58x50x48/46', // '46'=>'75x53x35/46',
            '47'=>'80x65x27/47', // 79x61x29/47
            '48'=>'75x55x35/48', // 0.144
            '49'=>'74x62x32/49',
            '50'=>'67x56x40/50', // 67*56*40/50
            '51'=>'90x65x26/51', // 78x65x30/51
            '52'=>'65x50x48/52',
            '53'=>'88x70x26/53',
            '54'=>'65x55x45/54',
            '55'=>'70x55x43/55',
            '56'=>'75x60x37/56',
            '57'=>'70x58x42/57',
            '58'=>'65x50x48/58', // 75x52x45/58
            '59'=>'80x60x37/59',
            '60'=>'74x57x43/60', // 63x48x60/60
            '61'=>'78x63x37/61', // 0.183
            '62'=>'90x57x36/62',
            '63'=>'74x57x45/63',
            '64'=>'95x58x35/64',
            '65'=>'73x62x43/65', // '65'=>'95x68x30/65',
            ...
            '84'=>'92x64x43/84',
            '92'=>'205x66x40/92',
			'91'=>'200x66x40/91', // 0.273 Это не точно
            '94'=>'75x65x58/94',
