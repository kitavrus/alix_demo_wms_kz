...
class OutboundRepository
{
    ...
    public function getOrdersForPrintPickList()
    {
        ...
        return new ActiveDataProvider([
            'query' => $query,
            'pagination' => false,
            'sort' => ['defaultOrder' => ['client_Priority' => SORT_DESC,'expected_qty' => SORT_ASC,'id' => SORT_DESC]]
        ]);
    }
    ...
    public function getOrderInfo($id)
    {
        ...
        $result->order = $order;
        $result->items = $items;
        $result->outboundBoxBarcode = EcommerceStock::find()->select('outbound_box')->andWhere(['outbound_id'=>$id])->scalar();
...
//            $order->external_order_number = $externalOrderNumber;
