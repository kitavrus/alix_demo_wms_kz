...
//        $dtoForCreateStock->apiStatus  = StockAPIStatus::YES;
    ...
    public function showNotEmptyProductBarcode($aProductRow) {

        $result = (!empty($aProductRow['product_barcode']) ? $aProductRow['product_barcode'] : "");
        $result .= (!empty($aProductRow['product_barcode1']) && $aProductRow['product_barcode'] != $aProductRow['product_barcode1']  ? " / ".$aProductRow['product_barcode1'] : "");
        $result .= (!empty($aProductRow['product_barcode2']) ? " / ".$aProductRow['product_barcode2'] : "");
        $result .= (!empty($aProductRow['product_barcode3']) ? " / ".$aProductRow['product_barcode3'] : "");
...
//            $this->returnRepository->ser
    ...
    // ------------------------------------------- отправляем фидбек как возврат по обычным заказам которые отменил клиент
    public function sendOutboundByAPI($orderInfo,$onlyStockId = [],$excludeStockId = []) {
        ...
        $productsReadyList = $this->prepareProductsOutboundReadyForSendByAPI($orderInfo,$onlyStockId);

        $returnAPIService = new ReturnAPIService();
        return $returnAPIService->send($productsReadyList,0);
//        return $productsReadyList;
    }

    public function prepareProductsOutboundReadyForSendByAPI($orderInfo,$onlyStockId) {

        $B2CReturnShipmentItems = [];
        foreach($orderInfo->items as $item) {

            if(!empty($onlyStockId) && !in_array($item->product_sku,$onlyStockId)) {
                continue;
            }
            ...
            $B2CReturnShipmentItems [] = [
                ...
                'ReturnReasonCode'=>ReturnReason::OTHER,
                'ReturnReasonProcessCode'=> ReturnOutbound::DONATION_VALUE,
            ];
        }

        $result = [
            'ExternalShipmentId'=>$orderInfo->order->order_number, //'OMC-8186455',
            'UniqueNumber'=>BarcodeManager::generateUuid(), // ? ,
            'CargoReturnCode'=> null, // ? ,
            'RefundUser'=> 'Kayrat', // ? ,
            'RefundUserId'=> '1', // ? ,
        ...
        if(empty($B2CReturnShipmentItems)) {
            return null;
        }
