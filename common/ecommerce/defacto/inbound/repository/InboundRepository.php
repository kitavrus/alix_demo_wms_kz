...
//    }
	...
	public function deleteZeroScannedBox($orderId)
	{
		$inboundItemClientBox = EcommerceInboundItem::find()
													->select('client_box_barcode')
													->andWhere(['inbound_id' => $orderId])
													->groupBy('client_box_barcode')
													->column();

		if (empty($inboundItemClientBox)) {
			return;
		}

		foreach ($inboundItemClientBox as $clientBox) {

			$scannedProductInBox = EcommerceStock::find()->andWhere([
				'inbound_id' => $orderId,
				'client_box_barcode'=>$clientBox,
				'client_id' => $this->getClientID(),
			])->exists();

			if(empty($scannedProductInBox)) {
				EcommerceInboundItem::deleteAll([
					'inbound_id' => $orderId,
					'client_box_barcode' => $clientBox,
				]);
			}
