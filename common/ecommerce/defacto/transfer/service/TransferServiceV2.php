...
class TransferServiceV2
{
    ...
    public function __construct() {
        ...
        $this->api = new \common\ecommerce\defacto\transfer\service\TransferAPIServiceV2();
    }
    ...
    public function GetBatches() {
		...
		//$batchId = 19187605;
           ...
           // VarDumper::dump($productsList,10,true);
            $this->saveTransferItem($productsList);
       }
		//die("-GetBatches end-");
        return $batchIdList;
    }

    public function prepareGetBatches($aResponseFromGetBatches) {
        $lcBarcodeList = [];
...
//                        }
						...
						$isExistTransfer = EcommerceTransfer::find()
															->andWhere([
																'client_BatchId' => ArrayHelper::getValue($dataListItem, 'BatchId')
															])->exists();
                        if($isExistTransfer) {
                            continue;
                        }
...
//                }
    ...
    private function GetOutBound($aBatchId) {
		...
		//$result = unserialize('a:4:{s:8:"HasError";b:0;s:12:"ErrorMessage";s:0:"";s:7:"Message";s:0:"";s:4:"Data";O:8:"stdClass":3:{s:8:"HasError";b:0;s:9:"IsSuccess";b:1;s:4:"Data";O:8:"stdClass":1:{s:14:"B2COutboundDTO";a:3:{i:0;O:8:"stdClass":6:{s:10:"OutboundId";i:101034313;s:7:"BatchId";i:19187605;s:5:"SkuId";i:228013772;s:8:"Quantity";s:4:"1.00";s:6:"Status";s:18:"ReadyforProcessing";s:16:"ToBusinessUnitId";i:1029;}i:1;O:8:"stdClass":6:{s:10:"OutboundId";i:101034314;s:7:"BatchId";i:19187605;s:5:"SkuId";i:228014295;s:8:"Quantity";s:4:"8.00";s:6:"Status";s:18:"ReadyforProcessing";s:16:"ToBusinessUnitId";i:1029;}i:2;O:8:"stdClass":6:{s:10:"OutboundId";i:101034315;s:7:"BatchId";i:19187605;s:5:"SkuId";i:228014294;s:8:"Quantity";s:4:"4.00";s:6:"Status";s:18:"ReadyforProcessing";s:16:"ToBusinessUnitId";i:1029;}}}}}');
       // file_put_contents("GetOutBound-serialize",serialize($result)."\n",FILE_APPEND);
...
//                        $productsList[$key]->ourTransferId = $transferMain->id;
						$item->OutboundId = ArrayHelper::getValue($dataListItem, 'OutboundId');
						$item->BatchId = ArrayHelper::getValue($dataListItem, 'BatchId');
						$item->SkuId = ArrayHelper::getValue($dataListItem, 'SkuId');
						$item->Quantity = ArrayHelper::getValue($dataListItem, 'Quantity');
						$item->Status = ArrayHelper::getValue($dataListItem, 'Status');
						$item->ToBusinessUnitId = ArrayHelper::getValue($dataListItem, 'ToBusinessUnitId');
						$productsList[$aBatchId][$item->ToBusinessUnitId][] = $item;
                    }
                }
            }
        }
    ...
    private function saveTransferItem($productsList) {
		if (empty($productsList) || !is_array($productsList)) {
			return;
		}

		foreach ($productsList as $BatchId=>$ToBusinessUnitIds) {
			foreach ($ToBusinessUnitIds as $ToBusinessUnitId=>$productItems) {
				foreach ($productItems as $productItem) {

				$transfer = EcommerceTransfer::find()
												->andWhere([
													'client_BatchId' => $BatchId,
													'client_ToBusinessUnitId' => $ToBusinessUnitId,
												])
												->one();
				if(!$transfer) {
					$transfer = new EcommerceTransfer();
					$transfer->client_id = 2;
					$transfer->client_BatchId = $productItem->BatchId;
					$transfer->client_Status = $productItem->Status;
					$transfer->client_ToBusinessUnitId = $ToBusinessUnitId;
					$transfer->status = TransferStatus::_NEW;
					$transfer->api_status = StockAPIStatus::NO;
					$transfer->save(false);
				}

				$transferItem = EcommerceTransferItems::find()->andWhere([
					'transfer_id' => $transfer->id,
					'client_OutboundId' =>  $productItem->OutboundId,
					'client_BatchId' =>  $productItem->BatchId,
					'client_SkuId' =>  $productItem->SkuId,
					'client_Quantity' =>  $productItem->Quantity,
					'client_ToBusinessUnitId' =>  $productItem->ToBusinessUnitId,
				])->one();
//
				if($transferItem) {
					continue;
					...
					$transferItem = new EcommerceTransferItems();
					$transferItem->transfer_id =$transfer->id;
					$transferItem->client_OutboundId = $productItem->OutboundId;
					$transferItem->client_BatchId = $productItem->BatchId;
					$transferItem->client_SkuId = $productItem->SkuId;
					$transferItem->client_Quantity = $productItem->Quantity;
					$transferItem->client_Status = $productItem->Status;
					$transferItem->client_ToBusinessUnitId = $productItem->ToBusinessUnitId;
					$transferItem->product_sku = $productItem->SkuId;
					$transferItem->expected_qty = $productItem->Quantity;
					$transferItem->status = TransferStatus::_NEW;
					$transferItem->api_status = StockAPIStatus::NO;
					$transferItem->save(false);

//					VarDumper::dump($productItem,10,true);
					$transfer->expected_qty += $productItem->Quantity;
					$transfer->save(false);
				}
			}
		}
