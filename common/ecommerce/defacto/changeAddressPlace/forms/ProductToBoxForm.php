...
class ProductToBoxForm extends Model
{
    ...
    public function validateProductBarcode($attribute, $params)
    {
        ...
        if(!$this->validation->isProductExistInBoxForInbound($productBarcode,$fromBox)) {
    ...
    {
        return [
            'fromBox' => Yii::t('inbound/forms', 'Из короба (прихода)'),
            'productBarcode' => Yii::t('inbound/forms', 'ШК товара (перемещается только один товар)'),
            'toBox' => Yii::t('inbound/forms', 'В короб (который на полке)'),
        ];
    }
