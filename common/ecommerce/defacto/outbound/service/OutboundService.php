...
//    }
	...
	public function makeScannedQRCode()
    {
        $this->repository->makeScannedStockQRCode($this->dto);
    }
...
//        die;
    ...
    public function GetShipmentsV2($OrderQuantity = 30) {
        $shipmentList = $this->api->GetShipmentsV2($OrderQuantity);
//        $shipmentList = EcommerceApiOutboundLog::find()->andWhere(['id'=>1])->one();
//        $shipmentList = unserialize($shipmentList->response_data);
//        VarDumper::dump(ArrayHelper::getValue($shipmentList,'Data'),10,true);
//        die;
    ...
    public function SendShipmentFeedback($ourOutboundId)
    {
        ...
        if($orderInfo->order->expected_qty != $orderInfo->order->accepted_qty) {
            $orderInfo->order->client_CancelReason = \common\ecommerce\constants\OutboundCancelStatus::UN_COLLECTED;
	...
	public function partialCancelShipment($ourOutboundId) {

		$orderInfo = $this->getOrderInfo($ourOutboundId);

		if($orderInfo->order->expected_qty == $orderInfo->order->accepted_qty) { return []; }

		$orderInfo->order->client_CancelReason = \common\ecommerce\constants\OutboundCancelStatus::PARTIAL_CANCEL;
		$orderInfo->order->save(false);

		$orderInfo = $this->getOrderInfo($ourOutboundId);
		return $this->api->PartialCancelShipment($orderInfo);
	}
    ...
    public function CancellationRequestExistsByCustomer($ourOutboundId) {
	    ...
	    public function PartialCancellationRequestExistsByCustomer($ourOutboundId) {
        $orderInfo = $this->getOrderInfo($ourOutboundId);
        $apiResponse = $this->api->PartialCancellationRequestExistsByCustomer($ourOutboundId,$orderInfo->order->order_number);
		...
		if (count($apiResponse['Data'][0]->CancellationRequestDetail->CancellationRequestDetail) == 1) {
			$cancelItems = [$apiResponse['Data'][0]->CancellationRequestDetail->CancellationRequestDetail];
		}
...
//        }
    ...
    public function reservationOrdersForPrintPickingList($outboundOrderIds) {
        ...
        foreach($outboundOrderIds as $id) {
			...
			$service->PartialCancellationRequestExistsByCustomer($id);
    ...
    public function CancelShipment($ourOutboundId,$reason)
    {
        ...
        if(!in_array($reason,array_keys(OutboundCancelStatus::getForPartReReservedList()))) {
...
//        $this->SendAcceptedShipments($dto->order->id);
        //if(!in_array($dto->order->id,[1218,1222,1235])) {
			
			//$cargoLabel = $this->GetCargoLabel($dto->order->id);
			
			// Этот метод переносится в лист отргузки когда ребята его будут печатать ( вурнули назад )))
...
//        file_put_contents($fullPathFileName,base64_decode($cargoLabel->FileData));
		...
		$req  = new SendCargoDeliveryIn();
		$req->orderId = $dto->order->id;
		$req->orderNumber = $dto->order->order_number;
		$req->courierCompany = $dto->order->client_CargoCompany;
		$req->cargoShipmentNo = $cargoLabel->TrackingNumber;
		$req->kg = $dto->kg;
		$req->trackingUrl = $cargoLabel->TrackingUrl;
        $this->SendCargoDelivery($req);
	...
	public function resendGetCargoLabel($aOrderNumber) {

        $order = $this->repository->getOrderByOrderNumber($aOrderNumber);

        $cargoLabel = $this->GetCargoLabel($order->id);
        ...
        $order->path_to_cargo_label_file = $pathToCargoLabelFile;
        $order->client_TrackingNumber = $cargoLabel->TrackingNumber;
        $order->client_TrackingUrl = $cargoLabel->TrackingUrl;
        $order->client_ReferenceNumber = $cargoLabel->ReferenceNumber;
        $order->save(false);

		$req  = new SendCargoDeliveryIn();
		$req->orderId = $order->id;
		$req->orderNumber = $order->order_number;
		$req->courierCompany = $order->client_CargoCompany;
		$req->cargoShipmentNo = $cargoLabel->TrackingNumber;
		$req->kg = $order->kg;
		$req->trackingUrl = $cargoLabel->TrackingUrl;
		$this->SendCargoDelivery($req);
...
//	public function SendCargoDelivery($ourOutboundId,$getCargoLabelResponse)
	{
//		$orderInfo = $this->getOrderInfo($ourOutboundId);
		$response = $this->api->SendCargoDelivery($sendCargoDeliveryIn);
		if($response['HasError'] == false) {
			$this->setApiStatusYes($sendCargoDeliveryIn->orderId);
		} else {
			$this->setApiStatusError($sendCargoDeliveryIn->orderId);
		}
