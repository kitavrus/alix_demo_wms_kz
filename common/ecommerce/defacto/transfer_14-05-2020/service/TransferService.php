...
//        $product->product_accepted_qty += 1;
//        $product->save(false);

        $printPickListInfo = $this->repository->getPrintPickListInfo($aDto->pickingListBarcode);
        $result = new \stdClass();
        $result->productExpectedQty = $product->expected_qty;
        $result->productAcceptedQty = $product->accepted_qty;

        $result->totalExpectedQty = $printPickListInfo->expected_qty;
    ...
    public function scannedPrintPickListBarcode($aDto) {
        ...
        $result = new \stdClass();
        $result->expectedQty = $printPickListInfo->expected_qty;
...
//        VarDumper::dump();
        ...
        return $this->repository->showBoxItems($aDto->lcBarcode,$aDto->pickingListBarcode);
    }
    ...
    public function emptyBox($aDto) {
        ...
        $printPickListInfo = $this->repository->getPrintPickListInfo($aDto->pickingListBarcode);
        $result = new \stdClass();
        $result->productExpectedQty = 0;
        $result->productAcceptedQty = 0;
