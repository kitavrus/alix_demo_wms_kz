...
//            'allModels' => $query->asArray()->all(),
	...
	public function searchForTransfer($params)
	{
		...
		$dataProvider = new ActiveDataProvider([
			...
			'sort' => ['defaultOrder' => ['scan_out_datetime' => SORT_DESC]]
		]);
...
//		}
		...
		$query->andWhere(["status_transfer" => 4]);
		$query->andWhere("transfer_id != '0'");

		$query->andFilterWhere(['like', 'product_barcode', $this->product_barcode])
			  ->andFilterWhere(['like', 'transfer_outbound_box', $this->transfer_outbound_box]);

		if(!empty($this->scan_out_datetime)) {
			$date = explode('/',$this->scan_out_datetime);
			$date[0] = trim($date[0]).' 00:00:00';
			$date[1] = trim($date[1]).' 23:59:59';

			$query->andWhere(['between', 'scan_out_datetime', strtotime($date[0]),strtotime($date[1])]);
		}

		if(!empty($this->transfer_id)) {
			$query->andWhere(["transfer_id" => $this->transfer_id]);
		}

		return $dataProvider;
	}

	public function getTransferById($aTransferId) {
		return EcommerceTransfer::find()->select('client_BatchId')->andWhere(["id" => $aTransferId])->scalar();
	}

	public function getLastTransfer() {

		return ArrayHelper::map(EcommerceTransfer::find()
								->select('client_BatchId,id')
								->andWhere('client_LcBarcode = "10virtual-box00"')
