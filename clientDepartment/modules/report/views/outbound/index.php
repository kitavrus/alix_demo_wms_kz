...
//        ''=>'',
        ...
        'columns' => [
//			[
//				'attribute'=> 'completeOrder',
//				'format'=> 'raw',
////				'noWrap'=> true,
//				'value' => function ($data) {
//					return Html::a('Completed ',
//						['completed', 'id' => $data->id],
//						[
//						  'class'=>'btn btn-danger',
//						  'data' => ['confirm' => "Вы действительно хотите сделать Completed заказу?",'method' => 'post'],
//						]
//					);
//				},
//			],
...
//            'kg',
            [
                ...
                'value' => function ($data) {
                    return ''
                        .$data->accepted_number_places_qty.' / '
                        .'<h4 class="pull-left">'.Html::tag('a', 'print boxes', ['href'=>Url::to(['print-box-kg-list', 'id' => $data->id]),'class'=>'label label-default','style'=>'margin:5px; display: block;'])
                        //."<br />"
                        .Html::tag('a', 'products in box', ['href'=>Url::to(['print-products-in-box', 'id' => $data->id]),'class'=>'label label-default','style'=>'margin:5px; display: block;'])
                        .'</h4>';
                },
            ],
            'expected_qty',
                    ...
                    if($model->cargo_status == OutboundOrder::CARGO_STATUS_DELIVERED || $model->cargo_status == OutboundOrder::CARGO_STATUS_ON_ROUTE){
                        $bt = \yii\helpers\Html::tag('span', Yii::t('transportLogistics/buttons','Export Outbound'),
                            [
                                ...
                                'data-url-value'=>Url::to(['/report/outbound/export-rn?id='.$model->id])
                            ]);
                    } else {
                        $bt = \yii\helpers\Html::tag('span', Yii::t('transportLogistics/buttons','Export Outbound'),
                            [
...
<div>
    ...
    <?= Html::tag('span',Yii::t('transportLogistics/buttons','Export to Excel'),['class' => 'btn btn-success','id'=>'report-order-export-btn', 'data-url'=>'/report/outbound/export-to-excel']) ?>
    <?= Html::tag('span',Yii::t('transportLogistics/buttons','Export to boxes Excel'),['class' => 'btn btn-warning','id'=>'report-order-export-boxes-btn', 'data-url'=>'/report/outbound/print-box-kg-list-by-parent']) ?>
	<?= Html::tag('span',Yii::t('transportLogistics/buttons','Complete'),['class' => 'btn btn-warning','id'=>'report-order-complete-api-btn', 'data-url'=>'/report/outbound/complete-api']) ?>
    <?php
//    if(\clientDepartment\modules\client\components\ClientManager::canExportRn()){
//        echo Html::tag('span',Yii::t('transportLogistics/buttons','Export Outbound'),['class' => 'btn btn-primary','id'=>'report-outbound-btn', 'data-url'=>'/report/outbound/export-rn']);
//    }
//    ?>
    <br />
</div>
...
<script type="text/javascript">
    $(function(){
        ...
        $('#report-order-export-boxes-btn').on('click',function() {
            window.location.href = $(this).data('url')+'?'+$('#report-search-form').serialize();
        });
		
		
        $('#report-order-complete-api-btn').on('click',function() {
            window.location.href = $(this).data('url')+'?'+$('#report-search-form').serialize();
        });
