...
//            $ids = [];
     ...
     *
     * */
    public function cityOrShopArrayData()
    {
        return ['city'=>Yii::t('kpi-delivered/titles','By city'),'shop'=>Yii::t('kpi-delivered/titles','By shop')];
    }

    /*
     *
     * */
    public function getCountryArrayData()
    {
        $data = \common\modules\city\models\Country::getArrayData();

        if(Yii::$app->language == 'tr') {
            foreach ($data as $i=>$name) {
                $data[$i] = Inflector::slug($name);
            }
        }
