...
$storeTitleTo = $routeTo->city->name. ' / ' . $routeTo->name . ' '.(!empty($routeTo->shop_code) ? $routeTo->shop_code : '').' '. ((!empty($routeTo->shopping_center_name) && $routeTo->shopping_center_name != '-')  ? '  [ ТЦ ' . $routeTo->shopping_center_name . ' ] ' : '') .'<br />' .$routeTo->street. ' '.$routeTo->house;;
...
if($routeFrom = $model->routeFrom) {

    if($clientFrom = ClientManager::getClientEmployeeByAuthUser()) {
        $managersNamesFrom = $clientFrom->first_name . ' ' . $clientFrom->last_name . ' / ' . $clientFrom->phone_mobile . ' ' . $clientFrom->phone;// . "<br />"
    }
}

$storeTitleFrom = $routeFrom->city->name. ' / ' . $routeFrom->name . ' '.(!empty($routeFrom->shop_code) ? $routeFrom->shop_code : '').' '. ((!empty($routeFrom->shopping_center_name) && $routeFrom->shopping_center_name != '-')  ? '  [ ТЦ ' . $routeFrom->shopping_center_name . ' ] ' : '') .'<br />' .$routeFrom->street. ' '.$routeFrom->house;;
...
//    $pdf->SetFont('dejavusans', '', 10);
    ...
    $pdf->MultiCell(100,0,
        ...
        $config_array[$i]['multicells'][1]['y']+5,
