...
<?= Html::tag('span',Yii::t('transportLogistics/buttons','Export to Excel With Orders'),['class' => 'btn btn-warning','id'=>'delivery-proposal-export-order-btn','style'=>'margin-left:10px;']) ?>
...
<script type="text/javascript">
    $(function(){
        ...
        $('#delivery-proposal-export-order-btn').on('click',function() {

            if($('tr[data-key]').length==0){
                alert('Нет записей для экспорта');
            } else {
                window.location.href = '/transportLogistics/tl-delivery-proposal/export-to-excel-with-order?'+$('#delivery-proposal-grid-view-filters').find('.form-control').serialize();
            }
