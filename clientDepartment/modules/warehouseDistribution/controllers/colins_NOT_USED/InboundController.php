...
use clientDepartment\modules\warehouseDistribution\models\ColinsProcessForm;
use yii\helpers\VarDumper;
                ...
                if ($model->file_1) {
                    ...
                    if (($handle = fopen($pathToCSVFile, "r")) !== FALSE) {
                        ...
                        while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
                            $row++;
                            if (count($data) != ColinsProcessForm::INBOUND_FILE_ROW_COUNT){
                                $session->remove('colinsOutboundFilePath');
                                Yii::$app->getSession()->setFlash('error', Yii::t('inbound/messages','Inbound file has incorrect number of columns'));
...
//                                }
                ...
                if ($model->file_2) {
                    ...
                    if (($handle = fopen($pathToCSVFile, "r")) !== FALSE) {
                        ...
                        fclose($handle);
                        $thead = isset ($fileData2[1]) ? $fileData2[1] : [];
                    ...
                    foreach ($fileData1 as $row=>$checkExistData){
                        if(isset($checkExistData['box_barcode'])){
                            if($exist = InboundOrder::find()->andWhere([
                                ...
                                ->andWhere(['not in', 'status', [Stock::STATUS_INBOUND_NEW, Stock::STATUS_INBOUND_COMPLETE]])
                                ->exists()
                            ){
...
//                   }
                ...
                if($fileData2){
                    if($double2){
                        //Проверка на дубли товаров во втором файле
                        $doubles =[];
    ...
    public function actionProcessConfirm()
    {
        ...
        $partyNumber += 7;
        $partyNumber++;
        $this->_partyNumber = 'Tir' . '-'. $partyNumber. '-' .date('Y');
        ...
        return $this->redirect('/warehouseDistribution/colins/inbound/index');
    }
                        ...
                        if($inbound = InboundOrder::find()->andWhere([
                            ...
                            if (isset($data['items']) && $data['items'] && is_array($data['items'])) {
                                ...
                                foreach ($data['items'] as $item) {
                                    ...
                                    //если товара нет в базе, то добавляем, если есть то обновляем
                                    ...
                                    if ($product->save(false)) {
                                        if(!ProductBarcodes::checkBarcodeExistsFromClient(Client::CLIENT_COLINS, $item['product_barcode'])){
...
//                    VarDumper::dump($storeArray,10,true);
//                    die('-stop addOutbound-');
...
//            VarDumper::dump($storeArray,10,true);
//            VarDumper::dump($fileDump,10,true);
//            die('addOutbound - fileDump');
            if ($fileDump) {
                            ...
                            if (isset($value['store']) && !empty($value['store'])) {
                                ...
                                foreach ($value['store'] as $storeId => $storeOrder) {
                                    if ($store = Store::findOne($storeId)) {
                                        ...
                                        if(!OutboundOrder::find()->andWhere([
                                            'client_id'=>   Client::CLIENT_COLINS,
