...
class OutboundController extends \clientDepartment\components\Controller
{
    ...
    public function actionUploadOutboundOrder()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            $dirPath = 'uploads/akmaral/outbound/' . date('Ymd') . '/' . date('Hi');
            ...
            if (file_exists($fileToPath)) {
                if (($handle = fopen($fileToPath, "r")) !== FALSE) {
                    ...
                    while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
                        ...
                        if ($row == 1) {
                            if (count($data) != AkmaralOutboundForm::FILE_COLUMN_QTY) {
                        ...
                        if ($row > 1) {
                            ...
                            $previewData[$row]['product_name'] = isset ($data[10]) ? iconv('windows-1251','utf-8',$data[10]) : "";
                ...
                fclose($handle);
                $session->set('akmaralOutboundFilePath', $fileToPath);
            }
    ...
    public function actionCreateOutboundOrder()
    {
        ...
        if($from_point_id && $to_point_id && file_exists($session->get('akmaralOutboundFilePath'))){
            $fileToPath = $session->get('akmaralOutboundFilePath');
            if (($handle = fopen($fileToPath, "r")) !== FALSE) {
                $row = 0;
                while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
                    $row++;
                    ...
                    if ($row > 1) {
                        ...
                        $tableRows[$row]['product_name'] = isset ($data[10]) ? iconv('windows-1251','utf-8',$data[10]) : "";
                    }
                }
                fclose($handle);
            }
            ...
            if($tableRows) {
                ...
                if($coo =  $oManager->createUpdateConsignmentOutbound()){
                    ...
                    if($oo = $oManager->createUpdateOutbound($data)){
                        ...
                        $oManager->createUpdateDeliveryProposalAndOrder(['change_price'=>TlDeliveryProposal::CHANGE_AUTOMATIC_PRICE_NO]);
                        $oManager->reservationOnStockByPartyNumber();

                        $session->remove('akmaralOutboundFilePath');
