...
use common\components\OutboundManagerErenRetail;
...
//					$dataMatrix = (string)$excelActive->getCell('G' . $i)->getValue();

					if ($productBarcode == null || $productQty == null) {
						continue;
					}
...
//						$order->items[$productBarcode]->dataMatrix[] = $dataMatrix;
	...
	public function actionCreateOutboundOrder()
	{
		...
		$partyNumber = Yii::$app->request->post('outbound_order_number');
		$orderNumber = Yii::$app->request->post('outbound_order_number');
		$tableRows = [];
		$order = new \stdClass();
		$order->totalQtyRows = 0;
		$order->expectedTotalProductQty = 0;
...
//				}
			...
			if($order->items) {
				$data = [];
				$oManager = new OutboundManagerErenRetail();
...
//					$data['consignment_outbound_order_id'] = $coo->id;
					...
					if($oo = $oManager->createUpdateOutbound($data)){
						...
						Stock::allocateAnyBarcodeByOutboundOrderIdErenRetail($oo->id);
			...
			if (file_exists($fileToPath)) {
				...
				if (($handle = fopen($fileToPath, "r")) !== false) {
					while (($data = fgetcsv($handle, 30000, ";")) !== false) {
						...
						if ($rowIndex > 1) {
							$data = array_filter($data, 'trim');
							file_put_contents("c.cccc",print_r($data,true)."\n",FILE_APPEND);
							$style = (string)$data[0];
							$color = isset($data[1]) ? (string)$data[1] : "color";
							$productName = (string)$data[2];
							$productBarcode = (string)$data[3];
							$productSize = (string)$data[4];
