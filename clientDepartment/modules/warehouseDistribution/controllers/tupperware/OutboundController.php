...
use app\modules\warehouseDistribution\models\TupperwareOutboundForm;
...
class OutboundController extends \clientDepartment\components\Controller
{
    ...
    {
        $model = new TupperwareOutboundForm();
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            $dirPath = 'uploads/tupperware/outbound/' . date('Ymd') . '/' . date('Hi');
            ...
            if (file_exists($fileToPath)) {
                if (($handle = fopen($fileToPath, "r")) !== FALSE) {
                    ...
                    while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
                        ...
                        if ($row == 1) {
                            if (count($data) != TupperwareOutboundForm::FILE_COLUMN_QTY) {
                ...
                fclose($handle);
                $session->set('tupperwareOutboundFilePath', $fileToPath);
            }
    ...
    public function actionCreateOutboundOrder()
    {
        ...
        if($from_point_id && $to_point_id && file_exists($session->get('tupperwareOutboundFilePath'))){
            $fileToPath = $session->get('tupperwareOutboundFilePath');
            if (($handle = fopen($fileToPath, "r")) !== FALSE) {
                $row = 0;
                while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
                    $row++;
            ...
            if($tableRows) {
                ...
                if($coo =  $oManager->createUpdateConsignmentOutbound()){
                    ...
                    if($oo = $oManager->createUpdateOutbound($data)){
                        ...
                        $session->remove('tupperwareOutboundFilePath');
