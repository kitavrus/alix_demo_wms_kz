...
use app\modules\warehouseDistribution\models\KotonOutboundForm;
...
class OutboundController extends \clientDepartment\components\Controller
{
    ...
    public function actionUploadOutboundOrder()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            $dirPath = 'uploads/cotton/outbound/' . date('Ymd') . '/' . date('Hi');
            ...
            if (file_exists($fileToPath)) {
                if (($handle = fopen($fileToPath, "r")) !== FALSE) {
                    ...
                    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                        ...
                        if ($row == 1) {
                            if (count($data) != KotonOutboundForm::FILE_COLUMN_QTY) {
                ...
                fclose($handle);
                $session->set('kotonOutboundFilePath', $fileToPath);
            }
    ...
    public function actionCreateOutboundOrder()
    {
        ...
        if($from_point_id && $to_point_id && file_exists($session->get('kotonOutboundFilePath'))){
            $fileToPath = $session->get('kotonOutboundFilePath');
            if (($handle = fopen($fileToPath, "r")) !== FALSE) {
                $row = 0;
                while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                    $row++;
            ...
            if($tableRows) {
                ...
                if($coo =  $oManager->createUpdateConsignmentOutbound()){
                    ...
                    if($oo = $oManager->createUpdateOutbound($data)){
                        ...
                        $session->remove('kotonOutboundFilePath');
