...
<!--    --><?php //= $form->field($model, 'store_id')->dropDownList($filterWidgetOptionDataRoute, ['prompt'=>Yii::t('return/titles', 'Select store'),'data'=>['url'=>Url::toRoute('get-inbound-orders-number')]]); ?>
    ...
    <?= Html::tag('span', Yii::t('return/buttons', 'Generate new'), ['data-url' => Url::toRoute('get-inbound-orders-number'), 'class' => 'btn btn-success', 'id' => 'inboundTupperware-process-form-generate-new-bt', 'style' => 'margin-left:10px;']) ?>
    <?= Html::tag('span', Yii::t('return/buttons', 'Delete order'), ['data-url' => Url::toRoute('delete-inbound-order'), 'class' => 'btn btn-danger hidden', 'id' => 'inboundTupperware-delete-inbound-order-bt', 'style' => 'margin-right:10px;']) ?>
    <?= Html::tag('span', Yii::t('return/buttons', 'Accept inbound order'), ['data-url' => Url::toRoute('accept-inbound-order'), 'class' => 'btn btn-warning pull-right', 'id' => 'inboundTupperware-process-form-accept-inbound-order-bt', 'style' => 'margin-right:10px;']) ?>
...
<br />
    ...
    <div class="row" style="margin: 20px 1px">
        <?= Html::submitButton(Yii::t('return/buttons', 'Upload').'<span id="inboundTupperware-process-form-upload-message"></span>', ['class'=>'btn btn-primary','id'=>'inboundTupperware-process-form-upload-submit-bt']) ?>
    </div>
    <?php ActiveForm::end(); ?>
...
<script type="text/javascript">
    $(function() {
        ...
        var b = $('body'),
            /*store = $("#tupperwareinboundform-store_id"),*/
            order = $("#tupperwareinboundform-inbound_order_number");
        ...
        b.on('click', '#inboundTupperware-process-form-generate-new-bt', function () {
               ...
               /* if (store_id) {*/
                    $.post(url, { 'generate_new': true})
                        .done(function (result) {
                            ...
                            $('#tupperwareinboundform-inbound_order_number').html('');

                            $.each(result.data_options, function (key, value) {
                                $('#tupperwareinboundform-inbound_order_number').append('<option value="' + key + '">' + value + '</option>');
                            });
                        })
                        .fail(function () {
                            console.log("server error");
...
/*                } else {
        ...
        b.on('click', '#inboundTupperware-process-form-accept-inbound-order-bt', function () {
            ...
            if (confirm('Вы действительно хотите отправить накладную на обработку складу')) {
                ...
                var me = $(this),
                    url = me.data('url'),
                    /*store_id = $('#returnform-store_id').val(),*/
                    party_id = $('#tupperwareinboundform-inbound_order_number').val();
                    ...
                    $.post(url, {/*'store_id': store_id,*/'party_id':party_id})
                        .done(function (result) {
                            if(result.errors == 0) {
                                ...
                                window.location.href = '/warehouseDistribution/tupperware/inbound/index';
                            } else {
                                window.location.href = '/warehouseDistribution/tupperware/inbound/index?&party_id='+result.party_id;
                            }
