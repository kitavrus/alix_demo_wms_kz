...
// Пунк погрузки
$loadingPoint = $model->routeFrom->city->name . ' / ' . $model->routeFrom->name;
$loadingPoint .= ((!empty($model->routeFrom->shopping_center_name) && $model->routeFrom->shopping_center_name != '-')  ? '  [ ТЦ ' . $model->routeFrom->shopping_center_name . ' ] ' : '');
// Пункт разгрузки
$unloadingPoint = $model->routeTo->city->name . ' / ' . $model->routeTo->name;
...
//    array('-','-','-','шт','-','-','-','-','Коробка',$numberPlaces,$kgActual,$numberPlaces,$kgActual,$mcActual),
);
