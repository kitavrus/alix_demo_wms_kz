...
use clientDepartment\modules\store\models\Store;
...
//        echo "<br />";
//        echo "<br />";
//        VarDumper::dump(['username'=>$value],10,true);
//        echo "<br />";
//        echo "<br />";
//        VarDumper::dump(User::findOne(['username'=>$value]),10,true);
        if(User::find()->where('username = :username AND id != :id',[':username'=>$value,':id'=>$this->user_id])->count()) {
            $this->addError($attribute, '['.$value.'] Менеджер с таким логином уже существует');
        }
    }
    /**
     * @inheritdoc
    ...
    {
        return [
            [['name','store_id','email','manager_type'], 'required'], // 'client_id',
...
//            ['name', 'required', 'on' => ['register', 'connect', 'create', 'update']],
