$(function() {
    ...
    var courierCompanyID = formID+'-couriercompany';
    var orderNumberID = formID+'-ordernumber';
    var courierCompanySelectedValue = '';
    ...
    b.on('click',barcodeID+", "+titleID+", "+orderNumberID,function(e) {
        console.info($(this).attr('id'));
        addFocusSelect(this);
    });


    b.on('change',courierCompanyID, function (e) {
        ...
        $.post(url, form.serialize(),function (result) {
            if (result.success == 0 ) {
                ...
                $(barcodeID).val('');
                $(orderNumberID).val('');
                courierCompanySelectedValue = $(me).val();

                console.info(courierCompanySelectedValue);

                if(courierCompanySelectedValue == 'Lamoda') {
    ...
    b.on('click',printBtID, function (e) {
        ...
        $.post(url, form.serialize(),function (result) {
            if (result.success == 0 ) {
                ...
                errorBase.hidden();
                showScannedQty(0);
                window.location.href = urlToRedirect+'?title='+result.title+'&courierCompany='+result.courierCompany;
            }
        }, 'json').fail(function (xhr, textStatus, errorThrown) {
        });
    });
    ...
    // SHOW ALL ORDERS BY COURIER
    b.on('click',showKaspiOrdersBtID, function (e) {
        e.preventDefault();
    ...
    // SHOW delete outbound list
    b.on('click',deleteListBt, function (e) {
        e.preventDefault();
        ...
        if (confirm("Вы действительно хотите удалить этот лист отгрзки?")) {
            $.post(url, form.serialize(),function (result) {
                if (result.success == 0 ) {
                    //errorBase.eachShow(result.errors);
