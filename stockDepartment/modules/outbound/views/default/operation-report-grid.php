...
//    ]
//); ?>
<?php echo $this->render('filters/_operation-report-grid-filter', ['model' => $searchModel,'clearRoute'=>['operation-report']]); ?>
...
<?= \yii\grid\GridView::widget([
    ...
    'rowOptions'=> function ($model, $key, $index, $grid) {
        $class = OutboundOrder::getGridColorByValue($model['status']);
        return ['class'=>$class];
    },
    'columns' => [
        'id',
//        ['class' => \yii\grid\SerialColumn::className()],
        ...
        [
            'label' => Yii::t('outbound/forms', 'Номер заказа'),
        ...
        [
            ...
            'value'=> function($model) {
                    if(isset($model['expected_qty']) && isset($model['allocated_qty'])) {
                        return $model['expected_qty'];
                    } else {
                        return $model['expected_number_places_qty'];
                    }
                }
        ],
       [
        'label' => Yii::t('outbound/forms', 'Зарезер-ое кол-во'),
        ...
        [
            ...
            'format'=>'raw',
            'value'=> function($model) {
                if(isset($model['accepted_qty']) && isset($model['allocated_qty'])) {
                    $ariaValueMin = 0;
            ...
            'value' => function ($model) use ($statusArray) {
                return ArrayHelper::getValue($statusArray,$model['status']);
            }
        ],
        [
            'label' => Yii::t('outbound/forms', 'Статус груза'),
            'attribute' => 'cargo_status',
            'value' => function ($model) use( $statusCargoArray) {
                 return ArrayHelper::getValue($statusCargoArray,$model['cargo_status']);
            }
        ],
        [
            'label' => Yii::t('outbound/forms', 'Специальный статус'),
            'attribute' => 'extra_status',
        ]
    ],
]); ?>
