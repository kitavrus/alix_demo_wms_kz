...
use stockDepartment\modules\outbound\models\OperationCrossDockSearch;
use stockDepartment\modules\outbound\models\OperationOutboundSearch;
use stockDepartment\modules\outbound\models\OperationInboundSearch;
...
//        $mm = new  MailManager();
//       if( $mm->sendErrorsMessageMail(['test'=>'test mail value '])) {
//           echo "OK";
//       }
    ...
    * */
    public function actionPrintPickList()
    {
        ...
        if($this->printType=='html'){
            ...
            return $this->render('_print-pick-list-html', ['items' => $items]);
        }
...
//        die('-----STOP-----');
        if($this->printType=='html'){
            Yii::$app->layout = 'print-html';
            return $this->render('printing-differences-list-html', ['items' => $items,'plIDs'=>$qIDs]);
        }
...
//            VarDumper::dump(ActiveForm::validate($model));
   ...
   /*             if ($dpOrder = TlDeliveryProposalOrders::findOne(['client_id' => $client_id, 'order_id' => $outboundModel->id, 'order_number' => $dpOrderNumber])) {
                ...
                if($dp = TlDeliveryProposal::find()
                    ...
                    if (!($dpOrder = TlDeliveryProposalOrders::findOne(['client_id' => $client_id, 'order_id' => $outboundModel->id, 'order_type'=>TlDeliveryProposalOrders::ORDER_TYPE_RPT, 'order_number' => $dpOrderNumber]))) {
                        $dpOrder = new TlDeliveryProposalOrders();
                    }
...
//                if ($items = OutboundOrderItem::findAll(['outbound_order_id' => $outbound->id])) {
    ...
    * TODO OLD NOT USED
    * */
    public function actionOperationReport_OLD_NOT_USED()
    {
...
//        $allocated_qty =
        $dataProvider->query->andWhere(['NOT IN','cargo_status',[OutboundOrder::CARGO_STATUS_DELIVERED]]);
    ...
    {
        $outboundSearch = new OperationOutboundSearch();
        $outboundDataProvider = $outboundSearch->search(Yii::$app->request->get());

        $outboundOnRoadQuery  = clone $outboundDataProvider->query;
        $outboundDataProvider->query->andWhere(['IN','cargo_status',[OutboundOrder::CARGO_STATUS_NEW,OutboundOrder::CARGO_STATUS_IN_PROCESSING,]]);
        $outboundOnRoadQuery->andWhere(['IN','cargo_status',[OutboundOrder::CARGO_STATUS_ON_ROUTE]]);

//        $inboundSearch = new OperationInboundSearch();
//        $inboundDataProvider = $inboundSearch->search(Yii::$app->request->get());
//        $inboundDataProvider->query->andWhere(['IN','order_type',[InboundOrder::ORDER_TYPE_INBOUND]]);
//        $inboundDataProvider->query->andWhere(['date_confirm'=>null]);
//        $inboundDataProvider->query->andWhere(['IN','status',[
...
//        ]]);
        ...
        $crossDockSearch = new OperationCrossDockSearch();
        $crossDockParams = null;
        if($get = Yii::$app->request->get('OperationOutboundSearch')) {
            $crossDockParams['OperationCrossDockSearch'] = $get;
        }

        $crossDockDataProvider = $crossDockSearch->search($crossDockParams);

        $crossDockOnRoadQuery  = clone $crossDockDataProvider->query;
        $crossDockDataProvider->query->andWhere(['IN','cargo_status',[OutboundOrder::CARGO_STATUS_NEW,OutboundOrder::CARGO_STATUS_IN_PROCESSING,]]);
        $crossDockOnRoadQuery->andWhere(['IN','cargo_status',[OutboundOrder::CARGO_STATUS_ON_ROUTE]]);

        $clientsArray = Client::getActiveItems();
        $statusArray = $outboundSearch->getStatusArray();
        $statusCargoArray = $outboundSearch->getCargoStatusArray();

        if($outboundSearch->order_type == 1) { // RPT
            $allModels = ArrayHelper::merge(
                $outboundDataProvider->query->asArray()->all(),
                $outboundOnRoadQuery->asArray()->all()
            );
        } elseif($outboundSearch->order_type == 2) { // CROSS-DOCK
            $allModels = ArrayHelper::merge(
                $crossDockDataProvider->query->asArray()->all(),
                $crossDockOnRoadQuery->asArray()->all()
            );
        } else {
            $allModels = ArrayHelper::merge(
                $outboundDataProvider->query->asArray()->all(),
//                $inboundDataProvider->query->asArray()->all(),
                $crossDockDataProvider->query->asArray()->all(),
                $outboundOnRoadQuery->asArray()->all(),
                $crossDockOnRoadQuery->asArray()->all()
            );

//            $allModels =  $inboundDataProvider->query->asArray()->all();
        }
        ...
        $arrayDataProvider->sort->defaultOrder = ['cargo_status'=>SORT_ASC,'created_at'=>SORT_DESC,];
...
//    }
    ...
    * @return JSON
    * */
    public function actionValidatePrintingBox()
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
        ...
        $items = Stock::find()
            ...
            ->andWhere([
                'or',
                'box_size_barcode=:box_size_barcode_1',
                'box_size_barcode=:box_size_barcode_2',
                'box_size_barcode=:box_size_barcode_3',
                'box_size_m3=:box_m3_1',
                'box_size_m3=:box_m3_2',
                'box_size_m3=:box_m3_3',

            ],
                [
                    ':box_size_barcode_1' => null,
                    ':box_size_barcode_2' => '',
                    ':box_size_barcode_3' => '0',
                    ':box_m3_1' => '0',
                    ':box_m3_2' => null,
                    ':box_m3_3' => ''
                ])
        ...
        if($items){
            $eMessages = '';
            foreach($items as $noSizeBox){
                $eMessages .= 'Для короба <b>['.$noSizeBox['box_barcode'].']</b> не установлен размер'.'<br>';
            }

            $model->addError('box_barcode', $eMessages);
            $errors = $model->getErrors();
        }
