...
//        $value = $this->box_barcode;
		...
		$pickingListBarcode= $this->picking_list_barcode;
        $orderId = Stock::find()->select("outbound_order_id")->andWhere([
			'outbound_picking_list_barcode'=>$pickingListBarcode,
		])->scalar();

        $isInOtherOrder = Stock::find()->where([
			'box_barcode'=>$value,
		])->andWhere("outbound_order_id != :outbound_order_id",[":outbound_order_id"=>$orderId])->exists();
		
		if($isInOtherOrder) {
			$this->addError($attribute, '<b>['.$value.']</b> '.Yii::t('outbound/errors','"Этот шк уже используется. Используйте новый шк короба'));
		}
		

/*        $plIds = !empty($this->picking_list_barcode_scanned) ?  $this->picking_list_barcode_scanned : '';

        if($plIds) {
            $qIDs = OutboundPickingLists::prepareIDsHelper($plIds);
//            VarDumper::dump($qIDs,10,true);
//            die;
            if (!empty($qIDs) && is_array($qIDs)) {
                ...
                $opl = Stock::find()->select('outbound_order_id')->where(['box_barcode' => $value])->andWhere(['in','status',[Stock::STATUS_OUTBOUND_SCANNING,Stock::STATUS_OUTBOUND_SCANNED]])->orWhere(['outbound_order_id'=>$outboundOrderIDs])->groupBy('outbound_order_id')->asArray()->count();
//                $opl = Stock::find()->select('outbound_order_id')->where(['box_barcode' => $value])->andWhere(['not in','status',[1]])->orWhere(['outbound_order_id'=>$outboundOrderIDs])->groupBy('outbound_order_id')->asArray()->one();

                    VarDumper::dump($opl,10,true);
            die;
                if ($opl >= 2) {
                    $this->addError($attribute, '<b>[' . $value . ']</b> ' . Yii::t('outbound/errors', 'Вы ввели штрихкод короба в котором есть товары из другого заказа'));
                }
            }
        }*/
        // b0000050305
    ...
    * */
    public function IsProductBarcode($attribute, $params)
    {
        ...
        if(BarcodeManager::isDefactoBox($value)) {
            if(!$this->isReturnBox($value) && !$this->isOneReturnBox($value)) {
    ...
    public function isOneReturnBox($returnBoxBarcode) {
       return  Stock::find()->andWhere([
            'client_id'=>Client::CLIENT_DEFACTO,
            'inbound_client_box' => $returnBoxBarcode,
