...
foreach ($outboundList as $outboundId => $productsInOrder) {
    ...
    $orderID = $productsInOrder[0]['order']['orderId'];//$productsInOrder[0]['order']['orderID'];// $order->id;
...
//    $currentDateTime = new DateTime("now", new DateTimeZone("Asia/Almaty"));
    ...
    $structure_table = '';
    if ($productsInOrder) {
        $headerText = '<span style="font-size: 4mm; font-weight: bold; ">Лист на сборку № <span style="font-size: 3mm; font-weight: bold; ">' . $orderNumber . '</span><br /><span style="font-size: 3mm;">' . $currentDateTime->format("d-m-Y H:i") . ' </span>';
        ...
        Stock::updateAll([
            'status_transfer' => \common\ecommerce\constants\StockTransferStatus::PRINTED_PICKING_LIST
        ],
            [
                'transfer_id' => PickingListRepository::prepareIDsHelper($orderID)
            ]);
...
$pdf->lastPage();
$pdf->Output($clientTitle.'-'.$orderNumber . '-pre-reserve.pdf', 'D');
Yii::$app->end();
