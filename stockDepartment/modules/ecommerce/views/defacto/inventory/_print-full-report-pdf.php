...
$structure_table = '<table width="100%" cellspacing="0" cellpadding="4" border="1">' .
    ...
    '      <th width="15%" align="center" valign="middle" border="1"><strong>' . Yii::t('outbound/forms','Старый короб') . '</strong></th>' .
...
//    '      <th width="20%" align="center" valign="middle" border="1"><strong>' . Yii::t('outbound/forms','Box barcode') . '</strong></th>' .
    '      <th width="15%" align="center" valign="middle" border="1"><strong>' . Yii::t('outbound/forms','Общее кол-во') . '</strong></th>' .
    '      <th width="20%" align="center" valign="middle" border="1"><strong>' . Yii::t('outbound/forms','Статус') . '</strong></th>' .
...
//            $structure_table .= '<tr align="center" valign="middle" style="background-color:'.($item['items'] == $item['count_status_scanned'] ? '#FFFFF1' : 'lightgray').'">';
            $structure_table .= '<tr align="center" valign="middle" style="background-color:'.'#FFFFF1'.'">';
            $structure_table .= '<td align="center" valign="middle" border="1">'.$item['place_address_barcode'].'</td>';
            $structure_table .= '<td align="center" valign="middle" border="1">'.$item['inventory_box_address_barcode'].'</td>';
            $structure_table .= '<td align="left" valign="middle" border="1">'.$item['product_barcode'].'</td>';
            $structure_table .= '<td align="center" valign="middle" border="1">'.$item['product_model'].'</td>';
            $structure_table .= '<td align="center" valign="middle" border="1">'.$item['product_qty'].'</td>';
            $structure_table .= '<td align="center" valign="middle" border="1">'.\common\ecommerce\entities\EcommerceInventory::getStatusScanValue($item['status_inventory']).'</td>';
//            $structure_table .= '<td align="center" valign="middle" border="1">'.$item['items'].'</td>';
//            $structure_table .= '<td align="center" valign="middle" border="1">'.$item['count_status_scanned'].'</td>';
            $structure_table .= '</tr>';

            $acc += $item['product_qty'];


//            $itemsProcess = \common\modules\stock\models\Stock::find()
//                ->select('id, product_barcode, box_address_barcode, place_address_barcode, box_barcode, product_model, count(*) as items ')
//                ->where([
...
$pdf->Output(date("d-m-Y-H-i-s") . '-full-report.pdf', 'D');
Yii::$app->end();