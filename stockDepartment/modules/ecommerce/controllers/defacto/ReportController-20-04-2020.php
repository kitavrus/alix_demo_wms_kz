...
//        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
    ...
    **/
    public function actionOutboundExportToExcelWithProducts()
    {
        ...
        $activeSheet->setCellValue('L' . $i, 'Cancel reason')->getColumnDimension('K')->setAutoSize(true);
...
//                    [['product_barcode','box_address_barcode','place_address_barcode'], 'string'],
    ...
    **/
    public function actionStockExportToExcel()
    {
        ...
        $i = 1;
        $activeSheet->setCellValue('A' . $i, 'Шк Товара')->getColumnDimension('A')->setAutoSize(true); // +; // +
        $activeSheet->setCellValue('B' . $i, 'Состояние товара')->getColumnDimension('B')->setAutoSize(true); // +; // +
        $activeSheet->setCellValue('C' . $i, 'Кол-во')->getColumnDimension('C')->setAutoSize(true); // +; // +

        $activeSheet->setCellValue('A' . $i, 'Product Barcode')->getColumnDimension('A')->setAutoSize(true); // +
        $activeSheet->setCellValue('B' . $i, 'Product condition')->getColumnDimension('B')->setAutoSize(true); // +
        $activeSheet->setCellValue('C' . $i, 'Product quantity')->getColumnDimension('C')->setAutoSize(true); // +
        $activeSheet->setCellValue('D' . $i, 'Box address')->getColumnDimension('D')->setAutoSize(true); // +
        $activeSheet->setCellValue('E' . $i, 'Place address')->getColumnDimension('E')->setAutoSize(true); // +
        $activeSheet->setCellValue('F' . $i, 'SKU')->getColumnDimension('E')->setAutoSize(true); // +
        ...
        foreach ($dps as $model) {
            ...
            $activeSheet->setCellValue('B' . $i, (new StockConditionType())->getConditionTypeValue($model['condition_type']) );
            $activeSheet->setCellValue('C' . $i, $model['qty']);
            $activeSheet->setCellValue('D' . $i, $model['box_address_barcode']);
            $activeSheet->setCellValue('E' . $i, $model['place_address_barcode']);
			$activeSheet->setCellValue('F' . $i, $model['client_product_sku']);
        }
...
//                file_put_contents($file,$row,FILE_APPEND);
                ...
                $activeSheet->setCellValue('C' . $i, $product['accepted_qty']);
            }
        }
