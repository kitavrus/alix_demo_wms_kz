...
class OutboundPickingListSearch extends OutboundPickingLists
{
    ...
    {
        return [
            [['client_id','status', 'created_user_id', 'updated_user_id', 'updated_at'], 'integer'],
            [['employee_barcode','barcode'], 'string'],
            [['order_number', 'parent_order_number','begin_datetime','created_at'], 'safe'],
    ...
    public function search($params)
    {
        ...
        if(!empty($this->employee_barcode)){
            $subQuery = Employees::find();
            $subQuery->andFilterWhere([
                'barcode' => $this->employee_barcode,
            ]);

            $ids = $subQuery->asArray()->column();
            $query->andFilterWhere(['employee_id'=> $ids]);
        }
