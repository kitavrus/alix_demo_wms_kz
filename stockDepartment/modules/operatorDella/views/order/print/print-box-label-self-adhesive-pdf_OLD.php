...
if(is_object($model->routeTo)){
    $recipient = $model->routeTo;
    $recipientText .= $recipient->contact_full_name.' / '.$recipient->phone_mobile." / ".$addRecipientPhone."<br />";
    $recipientText .= $recipient->title.', '.$recipient->floor.' этаж';
    $city = $recipient->city->name;
    $pointCode = $recipient->id;

}

if(is_object($model->routeFrom)) {
    $sender = $model->routeFrom;
    $senderText .= $sender->contact_full_name.' / '. $sender->phone_mobile." / ".$addRecipientPhone."<br />";
    $senderText .= $sender->title.', '.$sender->floor.' этаж';
}
...
//    $recipientText = '[DeFacto] ул. Гоголя 10, кв 11';
    $senderTitle = 'Отправитель';
//    $senderText = '[Nomadex] ул. Гоголя 10, кв 11';
    $ttnTitle = 'ТТН № ';
//    $ttn = '100-020-001-123';
...
//    $pdf->SetFont('dejavusans', '', 8);
    ...
    $htmlBox = '<table border="0" cellspacing="0" cellpadding="0"  style="line-height: 2; width: 100%;">' .
        ...
        . '</td>'
         ...
         . '<span style="font-weight: bold; ">Вес:  </span>' . $weight. ' кг'
         . '</td>'
        . '<td  style="text-align: center; width: 25%; "><span style="font-weight: bold; ">Мест: </span>'
        . $numberPlaces .
        '</td>'
        . '</tr>';
    $htmlBox .= '</table>';
