 ...
 * Time: 17:46
 */
namespace app\modules\intermode\controllers\ecommerce\barcode\domain\service;


use app\modules\intermode\controllers\ecommerce\inbound\domain\entities\EcommerceInboundPlaceBarcode;
use app\modules\intermode\controllers\ecommerce\outbound\domain\entities\EcommerceOutboundPackageBarcode;
...
//class OutboundPackageBarcodeService
{
	...
	public static function addGS($dataMatrix) {
		$gsCode = "";
		$gs = explode($gsCode,$dataMatrix);
		if(count($gs) == 1) {
			$qr = $dataMatrix;
			$part1 = substr($qr,0,31);
			$part2 = substr($qr,31,6);
			$part3 = substr($qr,37);
			return $part1.$gsCode.$part2.$gsCode.$part3;
		}
		return $dataMatrix;
	}
