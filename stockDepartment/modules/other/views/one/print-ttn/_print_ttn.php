...
// Пунк погрузки
$loadingPoint = $model->routeFrom->getPointTitleByPattern('{city_name}, {name} {street} {house}');
// Пункт разгрузки
$unloadingPoint = $model->routeTo->getPointTitleByPattern('{city_name}, {name} {street} {house}');
...
foreach($dataArray as $dataArrayKey => $row) {
    if($dataArrayKey==1) {
        if (!empty($outboundOrderItems)) {
            foreach ($outboundOrderItems as $orderValue) {
...
//                $kg += !empty($orderValue->kg_actual) ? $orderValue->kg_actual : $orderValue->kg;
                $sum += !empty($orderValue['acceptedQty']) ? $orderValue['acceptedQty'] : 0;

                $html_in .= '<tr>
                 <td width="7%">' . '-' . '</td>
                 <td width="7%">' . '-' . '</td>
                 <td width="16%">' . $orderValue['productName'].' '.($orderValue['productBarcode']). '</td>
                 <td width="7%">' . '-' . '</td>
                 <td width="7%">' . $orderValue['acceptedQty'] . '</td>
                 <td width="7%">' . '-' . '</td>
                 <td width="7%">' . '-' . '</td>
                 <td width="7%">' . '-' . '</td>
                 <td width="7%">' . '-' . '</td>
...
//        }
    ...
    $html_in.='<tr>
                 ...
                 <td width="7%">' . $row[3] . '</td>
                 <td width="7%">' . ($dataArrayKey==1 ? $sum :  $row[4] ) . '</td>
                 <td width="7%">' . $row[5] . '</td>
                 <td width="7%">' . $row[6] . '</td>
                 <td width="7%">' . $row[7] . '</td>
                 <td width="7%">' . $row[8] . '</td>
