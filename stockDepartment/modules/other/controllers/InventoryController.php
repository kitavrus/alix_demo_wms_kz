...
{
	private $inventoryId = 18;
//	private $clientId = Client::CLIENT_DEFACTO;
	private $clientId = 103;
...
//die("dfsdfsd");
		...
		for ($i = $start; $i <= 50000; $i++) {
			...
			$barcode = $excelActive->getCell('D' . $i)->getValue();
	...
	{
		die(" /other/inventory/add-pluses die begin");
		//$data = $this->loadPlusesFromFileXlsx("/web/inventory/intermode/2025-01-08/plusses.xlsx");
		$data = $this->loadPlusesFromFileXlsx("/web/inventory/intermode/2025-01-08/inbound-errors.xlsx");
		...
		foreach ($data as $box=>$products) {
			...
			foreach ($products as $product) {
				...
				$stock->system_status = 'inventoryInboundErrors20250108';
				$stock->system_status_description = 'это inbound-errors после инвентаризации 2025 01 08';
...
//		return $this->render("index", ["dataForVarDumper" => $products]);
	...
	public function actionBlockedLostLot()
	{
		...
		die("Все не найденное заблокировано успешно, инвент 18");

		$products = $this->findAllNotScannedLots();

		foreach ( $products as $product) {
			$product->status_availability = Stock::STATUS_AVAILABILITY_BLOCKED;
			$product->system_status = 'inventory-lost-20241209';
			$product->system_status_description = 'это не найденные лоты после инвентаризации 2022 12 09';
//
