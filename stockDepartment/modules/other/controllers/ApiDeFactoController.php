...
use yii;
use stockDepartment\modules\other\models\ApiDeFactoForm;
...
//                                    if(!isset($x[$inboundRow['KarsiDepoId']][$inboundRow['KoliId']])) {
                                    if(!isset($x[$inboundRow['KarsiDepoId']][$inboundRow['KoliKapatmaBarkod']])) {
                                        $x[$inboundRow['KarsiDepoId']][$inboundRow['KoliKapatmaBarkod']] = [
                                            'KarsiDepoId' => $inboundRow['KarsiDepoId'],
                                            'KoliId' => $inboundRow['KoliId'],
//                                            'KoliIdQty' => 0,
                                            'Desi' => !empty($inboundRow['Desi']) ? $inboundRow['Desi'] : '32',
...
/*                                $cd = new CrossDockLog();
                            ...
                            if($notFoundStores){
                                $shopCodes = implode(', ', $notFoundStores);
                                $messages = Yii::t('inbound/messages', 'Магазины с номерами [{shop-codes}] не найдены в системе. Загрузка невозможна', ['shop-codes' => $shopCodes]);
                                $updateStatus = 0;
                            }
//                            if ($io = CrossDock::findOne(['client_id' => $client_id, 'party_number' => $ioOrderNumber])) {
                            elseif ($io = ConsignmentCrossDock::findOne(['client_id' => $client_id, 'party_number' => $ioOrderNumber])) {
                                if (in_array($io->status, [
                ...
                if ($apiResponse = $api->getOutboundOrderByInvoice($model->invoice)) {
                    if (empty($apiResponse['errors'])) {
                        ...
                        foreach ($apiData as $key => $outboundRow) {
                            ...
                            $partyOrder = $outboundRow['PartiNo'];
                            ...
                            if (!($oul = OutboundUploadLog::findOne([
                                ...
                                if ($point = Store::find()->where(['client_id' => $oul->client_id, 'shop_code' => $shopId])->one()) {
                                    ...
                                    if(!in_array($notFoundStores, $shopId)){
                                        $notFoundStores[] = $shopId;
                                    }
                                }
                                //E: Find
...
//                        yii\helpers\VarDumper::dump($apiData,10,true);
                        ...
                        if($notFoundStores){
                            ...
                            $messages = Yii::t('outbound/titles', 'Если вы видете корректные данные, то нажмите кнопку "Загрузить в систему"');
                        }
