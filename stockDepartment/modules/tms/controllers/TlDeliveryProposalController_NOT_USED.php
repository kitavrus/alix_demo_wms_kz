...
use stockDepartment\modules\transportLogistics\models\CarModelPopup;
use stockDepartment\modules\transportLogistics\models\SelectSubRouteDefault;
use stockDepartment\modules\transportLogistics\models\TlDeliveryProposalSearchExport;
use yii\helpers\ArrayHelper;
//use common\modules\transportLogistics\models\TlDeliveryRouteOrders;
...
use stockDepartment\modules\transportLogistics\models\TlDeliveryProposalSearch;
use stockDepartment\modules\transportLogistics\models\SelectRouteCar;
...
//use stockDepartment\modules\transportLogistics\models\TlDeliveryProposalRouteCarsSearch;

//use frontend\modules\transportLogistics\models\TlOrder;
//use frontend\modules\transportLogistics\models\TlOrderItems;
...
//                $model->setCascadedStatus();
    ...
    public function actionCreate()
    {
        ...
        $model->company_transporter = TlDeliveryProposal::COMPANY_TRANSPORTER_NOMADEX;
...
//        }
     ...
     * */
    public function actionCopyOrder($proposal_id, $proposalOrderId)
    {
        $deliveryProposal = $this->findModel($proposal_id);
        $deliveryProposalOrder = TlDeliveryProposalOrders::findOne($proposalOrderId);

        if($deliveryProposal && $deliveryProposalOrder){
            $newDp = new TlDeliveryProposal();
            $newDp->setAttributes($deliveryProposal->getAttributes(null, ['id']), false);
            if( $newDp->save(false)){
//            if($newDp->validate() && $newDp->save()){
                $deliveryProposalOrder->tl_delivery_proposal_id = $newDp->id;
                $deliveryProposalOrder->save();
...
//        }
   ...
   * */
    ...
    public function actionGetRoutesByClient()
    {
        ...
        return [
            'message' => 'Success',
            'data_options' => TLHelper::getStockPointArray($currentClientID),
        ];
    }
