...
//
    ...
    $pageAllCurrent = 1;
    $batchCountAll = 37;
    foreach ($itemsProcessQuery->batch($batchCountAll) as $values) {
        $pageAll++;
    }
...
//                'status' => Stock::STATUS_OUTBOUND_RESERVED,
            ...
            ->andWhere('secondary_address LIKE "' . $level . '-%"')
            ->groupBy('product_barcode, primary_address')
            ->orderBy([
...
//                $PickingListBarcode = $orderNumber . '-' . $parentOrderNumber . '-' . $page;
                if ($orderNumber == $parentOrderNumber) {
                    ...
                    $PickingListBarcode = $orderNumber . '-' . $parentOrderNumber . '-' . $clientID . '-' . $page . '-' . $level;
                }
...
//                   $clientTitle = $outboundModel->client->title;
                ...
                $pdf->writeHTMLCell(110, 0, 10, 1, 'Лист на сборку № <span style="font-size: 5mm; font-weight: bold; ">' . $orderNumber . "</span> " . " [ " . $parentOrderNumber . " ] " . '<br /><span style="font-size: 5mm; font-weight: bold; ">' . $clientTitle . ' / ' . $shopToTitle . ' </span>', 0, 0, false, true, 'L');

                $pdf->SetFont('arial', 'B', 25);
                $pdf->writeHTMLCell(40, 0, 80, 0, "Этаж: ".$level, 1, 0, false, true, 'L');
                $pdf->SetFont('arial', 'B', 10);
                ...
                $pdf->write1DBarcode($PickingListBarcode, 'C128', '', '', 270, 16, 0.4, $style, 'M'); // T M B N
                ...
                $pdf->writeHTMLCell(0, 0, 0, $pdf->GetY() - 10, "Страниц: ".$page . ' из ' . $pages, 0, 0, false, true, 'R');
                $pdf->writeHTMLCell(0, 0, 0, $pdf->GetY() - 5,  "Всего стр: ".$pageAllCurrent++ . ' из ' . $pageAll, 0, 0, false, true, 'R');
