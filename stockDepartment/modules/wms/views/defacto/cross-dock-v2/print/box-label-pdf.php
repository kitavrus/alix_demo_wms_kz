...
//\yii\helpers\VarDumper::dump($boxTotal,10,true);
//die;

foreach ($boxes as $iBox=>$barcode) {

    $code = $barcode['order_number'];
	...
	$params['outboundOrderNumber'] = $orderModel->party_number.' / '.ltrim($orderModel->internal_barcode,'2-');
//	$params['outboundOrderNumber'] = $barcode['order_number'];

//	\yii\helpers\VarDumper::dump($params,10,true);
//	die;

	$pdf = \common\components\LabelPDFManager::OutboundBoxLabel($pdf,$params);

}

$pdf->lastPage();
$dirPath = 'uploads/box-labels/'.date('Ymd').'/'.date('His');
$fileName = Yii::$app->getSecurity()->generateRandomString(12). '-cross-dock-box-label.pdf';
...
if(file_exists($fullPath)){
	...
	$boxLabel->client_id = $orderModel->client_id;
	$boxLabel->outbound_order_id = $orderModel->id;
	$boxLabel->outbound_order_number = $orderModel->order_number;
