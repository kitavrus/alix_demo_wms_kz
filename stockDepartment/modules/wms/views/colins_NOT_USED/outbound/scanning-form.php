...
<?= $form->field($model, 'employee_barcode')->textInput([
    ...
    'data' => ['url'=>Url::toRoute('/wms/colins/outbound/employee-barcode-scanning-handler')]
]); ?>
...
<?= $form->field($model, 'order_shop',
    ...
    'data' => ['url'=>Url::toRoute('/wms/colins/outbound/order-shop-scanning-handler')]
])
?>
...
<?= $form->field($model, 'box_barcode', [
    ...
    'class' => 'form-control ext-large-input',
    'data' => ['url'=>Url::toRoute('/wms/colins/outbound/box-barcode-scanning-handler')]
...
['labelOptions' => ['label' => Yii::t('outbound/forms', 'Product Barcode')],
    ...
    '{input-group-begin}' => '<div class="input-group">',
    '{input-group-end}' => '</div>',
    '{button-right}' => '<div class="input-group-addon" style="background-color: none; border: none; border-radius: none;" ><span class="btn btn-success btn-xs" data-url-value="' . Url::toRoute(['clear-product-in-box']) . '" id="clear-product-in-box-by-one-scanning-outbound-bt">' . Yii::t('outbound/buttons', 'Clear product in box') . '</span></div>'
]
		]
)->textInput([
    'class' => 'form-control ext-large-input',
    'data' => ['url'=>Url::toRoute('/wms/colins/outbound/product-barcode-scanning-handler')]
...
<!--    --><?//= Html::tag('span', Yii::t('outbound/buttons', 'Print box contents'), ['data-url' => Url::toRoute('printing-box-content'), 'class' => 'btn btn-warning hidden', 'id' => 'scanning-colins-form-print-box-content-bt', 'style' => 'margin-right:10px; float:right;']) ?>
