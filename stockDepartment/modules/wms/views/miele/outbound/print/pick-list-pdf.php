...
//                'status' => Stock::STATUS_OUTBOUND_RESERVED,
    ...
    if ($count = $itemsProcessQuery->count()) {
        ...
        foreach ($itemsProcessQuery->batch($batchCount) as $values) {
            ...
            $shopToTitle = $order->to_point_title;

            $pdf->SetFont('arial', 'B', 10);
            $pdf->writeHTMLCell(110, 0, 10, 6, 'Лист на сборку № <span style="font-size: 5mm; font-weight: bold; ">' . $orderNumber . '</span><br /><span style="font-size: 5mm; font-weight: bold; ">' . $clientTitle . ' / ' . $shopToTitle . ' </span>', 0, 0, false, true, 'L');
