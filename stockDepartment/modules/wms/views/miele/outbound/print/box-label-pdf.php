...
$model = new stdClass();
$model->id = "123123123";
$model->number_places_actual = "12";
$model->number_places = "12";
...
$senderText = 'Miele:Nomadex Stock';//$routeFrom->city->name. ' / ' . $routeFrom->name . ' '.(!empty($routeFrom->shop_code) ? $routeFrom->shop_code : '').' '. ((!empty($routeFrom->shopping_center_name) && $routeFrom->shopping_center_name != '-')  ? '  [ ТЦ ' . $routeFrom->shopping_center_name . ' ] ' : '') . " " . $routeFrom->street . ' ' . $routeFrom->house;;
...
//
//}
$clientId = $dto->order->client_id;
$outboundOrderId = $dto->order->id;
$outboundOrderNumber = $dto->order->order_number;
$city = "ГОРОД";
$outboundOrderNumber = $clientId.'-'.$outboundOrderNumber;
...
foreach ($boxes as $iBox=>$barcode) {
    ...
    $ourBoxBarcode = $barcode['box_barcode'];
//	$code = $mappingOurBobBarcodeToDefacto[$code];

	$currentBoxNumber = $iBox+1;
	$codeBase = \common\components\BarcodeManager::createBaseBarcode($currentBoxNumber,$code,$ttn,$boxTotal);

	$params = [];
	$params['city'] = $city.(!empty($routeTo->city_prefix) ? '-'.$routeTo->city_prefix : '');
	...
	$params['ourBoxBarcode'] = $ourBoxBarcode;
...
if(file_exists($fullPath)){
	...
	$boxLabel->client_id = $clientId;
	$boxLabel->outbound_order_id = $outboundOrderId;
	$boxLabel->outbound_order_number = $outboundOrderNumber;
