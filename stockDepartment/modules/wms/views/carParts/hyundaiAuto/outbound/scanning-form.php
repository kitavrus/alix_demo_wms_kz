...
\stockDepartment\modules\wms\assets\carParts\main\ScanOutboundFormAsset::register($this);

?>
<h1>Отгрузка Hyundai Auto</h1>
...
<?= Html::a(Yii::t('buttons', 'ТТНки'),['/wms/carParts/main/report/delivery-order'], ['class' => 'btn btn-default','style' => 'margin-top:-42px; margin-left:22px; float:right; font-size: 25px;']) ?>
...
<div class="scanning-form">
    ...
    <?= $form->field($model, 'pick_list_barcode'
        ...
        ['template' => "{label}\n{input-group-begin}{button-right}{input}{input-group-end}\n{hint}\n{error}\n",
            'parts' => [
                ...
                '{input-group-end}' => '</div>',
                '{button-right}' => '<div class="input-group-addon" style="font-size: 62px;" id="pick-list-barcode-qty">0/0</div>'
            ]
        ]
    )->textInput([
    ...
    <?= $form->field($model, 'box_barcode'
        ...
        ['template' => "{label}\n{input-group-begin}{button-right}{input}{input-group-end}\n{hint}\n{error}\n",
            'parts' => [
                '{input-group-begin}' => '<div class="input-group">',
                '{input-group-end}' => '</div>',
                '{button-right}' => '<div class="input-group-addon" style="font-size: 62px; background-color: lightblue;" id="box-barcode-qty">0</div>'
            ]
        ]
    )->textInput([
        'class' => 'form-control ext-large-input',
    ...
    <?= $form->field($model, 'product_qty'
        ...
        'class' => 'form-control ext-large-input',
        'data-url' => Url::toRoute('product-qty-handler')
    ])->label(Yii::t('outbound/forms', 'Product Qty')) ?>
