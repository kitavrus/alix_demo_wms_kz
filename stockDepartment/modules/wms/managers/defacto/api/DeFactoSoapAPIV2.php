...
class DeFactoSoapAPIV2 {
    ...
    const INBOUND_STATUS_NOTHING = 'Nothing'; // новый
    const INBOUND_STATUS_MARKED_FOR_INBOUND_DATA = 'MarkedforInBoundData'; // В процессе подготовки. Ждем статус: InBoundDataIsPrepared
    const INBOUND_STATUS_DATA_IS_PREPARED = 'InBoundDataIsPrepared'; // 'Можно использовать метод GetAppointmentInBoundData
    const INBOUND_STATUS_COMPLETED = 'Completed'; // 'Заказ выполнен

    const OUTBOUND_STATUS_NEW = 'New'; // новый
    const OUTBOUND_STATUS_DATA_IS_PREPARED = 'OutBoundDataIsPrepared';

    const RETURN_STATUS = 'MixReturn';

    /*
     * @var string url
     * */
//    private $_url = 'http://195.46.145.251/DFStore.ProxyServices.UAT/ExternalWMS/ExternalWMSProxy.asmx?WSDL';
//    private $_urlTmp = 'http://195.46.145.251/DFStore.ProxyServices.UAT/ExternalWMS/ExternalWMSProxy.asmx';
//    private $_url = 'http://31.145.4.213/DFStore.ProxyServices/ExternalWMS/ExternalWMSProxy.asmx?WSDL';
//    private $_urlTmp = 'http://31.145.4.213/DFStore.ProxyServices/ExternalWMS/ExternalWMSProxy';
//    private $_url = 'http://10.0.5.1/DFStore.ProxyServices/ExternalWMS/ExternalWMSProxy.asmx?WSDL';
//    private $_urlTmp = 'http://10.0.5.1/DFStore.ProxyServices/ExternalWMS/ExternalWMSProxy.asmx';
    //private $_urlTmp = 'http://31.145.4.213/DFStore.ProxyServices/ExternalWMS/ExternalWMSProxy.asmx';
//    private $_url = 'http://195.46.145.251/DFStore.ProxyServices.UAT/ExternalWMS/ExternalWMSProxy.asmx?WSDL';
//    private $_urlTmp = 'http://195.46.145.251/DFStore.ProxyServices.UAT/ExternalWMS/ExternalWMSProxy';
//    private $_url = 'http://31.145.4.213/DFStore.ProxyServices/ExternalWMS/ExternalWMSProxy.asmx?WSDL'; // LIVE
    //private $_url = 'http://31.145.4.217/ExternalWMS/ExternalWMSProxy.asmx?WSDL'; // LIVE 2
//    private $_url = 'http://195.46.145.251/DFStore.ProxyServices.UAT/ExternalWMS/ExternalWMSProxy.asmx?WSDL'; // TEST RETURN
//    private $_url = 'http://195.46.145.251/DFStore.ProxyServices.UAT/ExternalWMS/ExternalWMSProxy.asmx?WSDL'; // DEMO TEST

		private $_url = 'https://externalwms.defacto.com/ExternalWMS/ExternalWMSProxy.asmx?WSDL'; // LIVE
...
//            'Password'=>'123456', // TEST
    ...
    * */
    ...
    public function sendRequest($method,$params,$soapHeader = null)
    {
        ...
        $dirPath = 'api/de-facto-new/all/'.date('Ymd');
        BaseFileHelper::createDirectory($dirPath);

		if ("GetMasterData" != $method) {
			file_put_contents($dirPath."/method-".$method."1.log","\n"."\n"."\n",FILE_APPEND);
			file_put_contents($dirPath."/method-".$method."1.log",date('Ymd-H:i:s')."\n"."\n",FILE_APPEND);
			file_put_contents($dirPath."/method-".$method."1.log",print_r($params,true)."\n"."\n",FILE_APPEND);
			file_put_contents($dirPath."/method-".$method."1.log",print_r($return,true)."\n",FILE_APPEND);
		}
...
//        die('-SendInBoundFeedBackData-');
     ...
     * */
    public function getMasterData($SkuId = null,
                                  $Ean = null,
                                  $ShortCode = null,
