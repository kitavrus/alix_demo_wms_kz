...
namespace stockDepartment\modules\wms\managers\erenRetail\checkBox\repository;
...
use stockDepartment\modules\wms\managers\erenRetail\checkBox\entities\CheckBoxInventory;
...
//        }
		...
		public function getAllBoxesFromStockByAddress($address)
	{
		...
		return Stock::find()
								 ->select('primary_address, COUNT(primary_address) as productQty, secondary_address') //
								 ->andWhere(["status_availability" => 2])
								 ->andWhere("secondary_address LIKE '" . $address . "%'")
								 ->groupBy('primary_address, secondary_address') //
								 ->orderBy(['primary_address' => SORT_DESC])
								 ->asArray()
								 ->all();
	}
}