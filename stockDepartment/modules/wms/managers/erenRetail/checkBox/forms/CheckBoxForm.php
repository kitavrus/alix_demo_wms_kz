...
namespace stockDepartment\modules\wms\managers\erenRetail\checkBox\forms;
...
class CheckBoxForm extends Model
{
    ...
    public function __construct($config = [])
    {
        ...
        $this->validation = new \stockDepartment\modules\wms\managers\erenRetail\checkBox\validation\Validation();
    ...
    * */
    ...
    public function PlaceAddress($attribute, $params)
    {
		...
		if(!$this->validation->palaceAddress($value)) {
            $this->addError($attribute, '<b>[' . $value . ']</b> ' . Yii::t('inbound/errors', 'Это не адрес  Б2Б этажа'));
            return;
        }
		*/
...
//        }
    ...
    * */
    public function ProductBarcode($attribute, $params)
    {
        ...
        if(!$this->validation->isProductBarcode($productBarcode,[$placeAddress],$boxBarcode)) {
    ...
    * */
    ...
    {
        $this->productBarcode = $this->checkBoxService->preLoadProductBarcode($this->productBarcode,$this->boxBarcode,$this->placeAddress);
