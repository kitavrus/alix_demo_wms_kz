...
//--------------------------------------------------
use common\b2b\domains\outboundLogitrans\repository\OutboundOrderSearch;
use common\b2b\domains\outboundLogitrans\forms\OutboundPickListForm;
use common\b2b\domains\outboundLogitrans\forms\ScanningForm;


class OutboundLogiTransController extends Controller
{
    /*
...
//        $searchModel = new \common\b2b\domains\outboundLogitrans\repository\OutboundOrderSearch();
...
//            'dataProvider' => $dataProvider,
    ...
    public function actionGetParentOrderNumber()
    {
        ...
        $data = [0 => 'Веберите партию'];
        $logiTransService  = new OutboundLogiTransService();
        $data += $logiTransService->getActiveParentOrderNumberByClientId($client_id);
...
//            ],
        ...
        $activeSheet = $objPHPExcel
            ...
            ->setTitle('picking-' . date('d.m.Y'));

        $i = 1;
//        $activeSheet->setCellValue('A' . $i, 'Клиент')->getColumnDimension('A')->setAutoSize(true); // +
//        $activeSheet->setCellValue('B' . $i, 'Родительский номер заказа')->getColumnDimension('B')->setAutoSize(true); // +
//        $activeSheet->setCellValue('C' . $i, 'Номер заказа')->getColumnDimension('C')->setAutoSize(true); // +
//        $activeSheet->setCellValue('D' . $i, 'Куда')->getColumnDimension('D')->setAutoSize(true); // +
//        $activeSheet->setCellValue('E' . $i, 'Штрих код товара')->getColumnDimension('E')->setAutoSize(true); // +
//        $activeSheet->setCellValue('F' . $i, 'Предполагаемое кол-во')->getColumnDimension('F')->setAutoSize(true); // +
//        $activeSheet->setCellValue('G' . $i, 'Зарезервированое кол-во')->getColumnDimension('G')->setAutoSize(true); // +
//        $activeSheet->setCellValue('H' . $i, 'Дата регистрации заказа')->getColumnDimension('H')->setAutoSize(true); // +
//        $activeSheet->setCellValue('I' . $i, 'SkuID')->getColumnDimension('I')->setAutoSize(true); // +
//        $activeSheet->setCellValue('J' . $i, 'LC Box')->getColumnDimension('J')->setAutoSize(true); // +
//        $activeSheet->setCellValue('K' . $i, 'LC Box')->getColumnDimension('K')->setAutoSize(true); // +

        $activeSheet->setCellValue('A' . $i, 'Родительский номер заказа')->getColumnDimension('A')->setAutoSize(true); // +
        $activeSheet->setCellValue('B' . $i, 'Номер заказа')->getColumnDimension('B')->setAutoSize(true); // +
        $activeSheet->setCellValue('C' . $i, 'Куда')->getColumnDimension('C')->setAutoSize(true); // +
        $activeSheet->setCellValue('D' . $i, 'LC Box')->getColumnDimension('D')->setAutoSize(true); // +


        $this->printPikingList($dps);
...
//            $activeSheet->setCellValue('H' . $i, !empty ($model->created_at) ? Yii::$app->formatter->asDatetime($model->created_at) : '-');
            ...
            foreach($items as $item) {
                ...
                $placeAddress = Stock::find()->select('secondary_address')
                    ->andWhere(['client_id'=>$model->client_id,'product_barcode'=>$item->product_barcode,'outbound_order_item_id'=>$item->id])
                ...
                $parentOrderNumber = $model->parent_order_number;
                ...
                $activeSheet->setCellValue('D' . $i, Stock::find()->select('inbound_client_box')->andWhere(['client_id'=>$model->client_id,'product_barcode'=>$item->product_barcode,'outbound_order_item_id'=>$item->id])->scalar());

//                $activeSheet->setCellValue('A' . $i, $clientTitle);
//                $activeSheet->setCellValue('B' . $i, $model->parent_order_number);
//                $activeSheet->setCellValue('C' . $i, $model->order_number);
//                $activeSheet->setCellValue('D' . $i, $title);
//                $activeSheet->setCellValue('E' . $i, $item->product_barcode);
//                $activeSheet->setCellValue('F' . $i, $item->expected_qty);
//                $activeSheet->setCellValue('G' . $i, $item->allocated_qty);
//                $activeSheet->setCellValue('I' . $i, Stock::find()->select('field_extra1')->andWhere(['client_id'=>$model->client_id,'product_barcode'=>$item->product_barcode,'outbound_order_item_id'=>$item->id])->scalar());
//                $activeSheet->setCellValue('J' . $i, Stock::find()->select('inbound_client_box')->andWhere(['client_id'=>$model->client_id,'product_barcode'=>$item->product_barcode,'outbound_order_item_id'=>$item->id])->scalar());
//                $activeSheet->setCellValue('K' . $i, $placeAddress);
            }
        }

        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="outbound-picking-list-' . $parentOrderNumber . '.xlsx"');
    ...
    * */
    ...
    public function actionBoxBarcodeScanningHandler()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            if( OutboundBoxService::isClientBoxByBarcode($model->box_barcode)) {
                $clientBox = $model->box_barcode = (new OutboundBoxService())->getOurBoxByBarcode($model->box_barcode);
            }
...
//            die; // 16,17,19
            if(BarcodeManager::isDefactoBox($model->product_barcode)) {
                if($productStockBarcode = Stock::find()->select('product_barcode')->andWhere([
                    'status' => [
                        Stock::STATUS_OUTBOUND_PICKED,
                        Stock::STATUS_OUTBOUND_SCANNING
                    ],
                    'inbound_client_box' => $model->product_barcode,
                    'outbound_picking_list_id' => $qIDs
                ])->scalar()) {
                    $model->product_barcode = $productStockBarcode;
                };
            }

            if(BarcodeManager::isBoxOnlyOur($model->product_barcode)) {
                if($productStockBarcode = Stock::find()->select('product_barcode')->andWhere([
                    'status' => [
                        Stock::STATUS_OUTBOUND_PICKED,
                        Stock::STATUS_OUTBOUND_SCANNING
                    ],
                    'primary_address' => $model->product_barcode,
                    'outbound_picking_list_id' => $qIDs
                ])->scalar()) {
                    $model->product_barcode = $productStockBarcode;
                };
...
//        die('---STOP--');
        ...
        if (isset($items[0]['outbound_order_id'])) {
            ...
            if ($dpo = TlDeliveryProposalOrders::findOne(['order_id' => $items[0]['outbound_order_id'],'order_type'=>TlDeliveryProposalOrders::ORDER_TYPE_RPT])) {
...
//            die($modelDP);
            ...
            if($outboundOrderModel->client_id == Client::CLIENT_DEFACTO && YII_ENV == 'prod' && false) { // id = 2 Дефакто
...
//                    StockExtraField::deleteAllById(Stock::find()->select('id')->andWhere(['outbound_order_id'=>$outboundOrderModel->id])->column());
                    file_put_contents('actionPrintingBoxLabel.csv',print_r("START",true)."\n"."\n",FILE_APPEND);
//                    file_put_contents('actionPrintingBoxLabel.csv',print_r($outboundOrderItems,true)."\n"."\n",FILE_APPEND);
                    foreach ($outboundOrderItems as $outboundOrderItem) {
                        ...
                        file_put_contents('actionPrintingBoxLabel.csv',print_r($stocks,true)."\n"."\n",FILE_APPEND);

                        //die("cccc-3");
                        if($tmp = DeFactoSoapAPIV2Manager::preparedSendOutBoundFeedBackData($stocks,$outboundOrderItem)) {
                            //die("cccc-4");
                            file_put_contents('actionPrintingBoxLabel.csv',print_r($tmp,true)."\n"."\n",FILE_APPEND);
...
//                                    }
//                                    die("cccc-5");
                                    file_put_contents('actionPrintingBoxLabel.csv',print_r("cccc-5",true)."\n"."\n",FILE_APPEND);
                                    file_put_contents('actionPrintingBoxLabel.csv',print_r($mappingOurBobBarcodeToDefacto,true)."\n"."\n",FILE_APPEND);
                                    file_put_contents('actionPrintingBoxLabel.csv',print_r($mappingWaybillNumber,true)."\n"."\n",FILE_APPEND);
                                }
                            }
//                            die("cccc-6");
                            file_put_contents('actionPrintingBoxLabel.csv',print_r("cccc-6",true)."\n"."\n",FILE_APPEND);
                        }
//                        die("cccc-7");
                        file_put_contents('actionPrintingBoxLabel.csv',print_r("cccc-7",true)."\n"."\n",FILE_APPEND);
...
//                        die("cccc-8");
                        file_put_contents('actionPrintingBoxLabel.csv',print_r("cccc-8",true)."\n"."\n",FILE_APPEND);
                        file_put_contents('actionPrintingBoxLabel.csv',print_r($stocks,true)."\n"."\n",FILE_APPEND);
                    ...
                    file_put_contents('actionPrintingBoxLabel.csv',print_r("cccc-9",true)."\n"."\n",FILE_APPEND);
                    file_put_contents('actionPrintingBoxLabel.csv',print_r($rowsDataForAPI,true)."\n"."\n",FILE_APPEND);
//                    die("cccc-9");
...
//                        die;
                       ...
                       // file_put_contents('SendOutBoundFeedBackDataSerializezzzzzz.csv',print_r($rowsDataForAPI,true)."\n",FILE_APPEND);
                        file_put_contents('SendOutBoundFeedBackDataSerialize.csv',serialize($rowsDataForAPI)."\n",FILE_APPEND);
                    }
                }
        ...
        file_put_contents('actionPrintingBoxLabel.csv',print_r("cccc-10",true)."\n"."\n",FILE_APPEND);
        file_put_contents('actionPrintingBoxLabel.csv',print_r($items,true)."\n"."\n",FILE_APPEND);
        file_put_contents('actionPrintingBoxLabel.csv',print_r($mappingOurBobBarcodeToDefacto,true)."\n"."\n",FILE_APPEND);
     ...
     * @param string $plids Picking list IDs
     * */
    public function actionRePrintingBoxLabel()
    {
        //11404260-37390-1
        $plIDs = Yii::$app->request->get('plids');
...
//                    StockExtraField::deleteAllById(Stock::find()->select('id')->andWhere(['outbound_order_id'=>$outboundOrderModel->id])->column());
//                    file_put_contents('actionPrintingBoxLabel-re.csv',print_r("START",true)."\n"."\n",FILE_APPEND);
//                    file_put_contents('actionPrintingBoxLabel.csv',print_r($outboundOrderItems,true)."\n"."\n",FILE_APPEND);
                    foreach ($outboundOrderItems as $outboundOrderItem) {
                        $stocks = Stock::find()->select('product_barcode, inbound_order_id, count(id) as accepted_qty, box_barcode, box_size_m3, box_size_barcode, outbound_order_item_id')
                            ->andWhere(['outbound_order_item_id' => $outboundOrderItem->id]) // ,'status'=>[Stock::STATUS_OUTBOUND_PRINTING_BOX_LABEL, Stock::STATUS_OUTBOUND_SCANNED]
...
//                        file_put_contents('actionPrintingBoxLabel-re.csv',print_r($stocks,true)."\n"."\n",FILE_APPEND);

                        //die("cccc-3");
                        if($tmp = DeFactoSoapAPIV2Manager::preparedSendOutBoundFeedBackData($stocks,$outboundOrderItem)) {
                            //die("cccc-4");
//                            file_put_contents('actionPrintingBoxLabel-re.csv',print_r($tmp,true)."\n"."\n",FILE_APPEND);
                            foreach($tmp as $tmpAPIValue) {
                                if(!isset($mappingOurBobBarcodeToDefacto[$tmpAPIValue['LcBarcode']])) {
                                    $mappingOurBobBarcodeToDefacto[$tmpAPIValue['LcBarcode']] = (new OutboundBoxService())->getClientBoxByBarcode($tmpAPIValue['LcBarcode']);
                                    $mappingWaybillNumber[$tmpAPIValue['LcBarcode']] = DeFactoSoapAPIV2Manager::makeWaybillNumber($outboundOrderModel,$boxCountStep);
                                    $boxCountStep++;
                                    file_put_contents("mappingOutboundBarcodeToDefacto-re.csv",
                                        $outboundOrderModel->id.";"
                                        .$tmpAPIValue['LcBarcode'].";"
                                        .$mappingOurBobBarcodeToDefacto[$tmpAPIValue['LcBarcode']].";"
                                        .$mappingWaybillNumber[$tmpAPIValue['LcBarcode']].";"
                                    ...
                                    OutboundBoxService::addLcAndWaybillByBoxBarcode($tmpAPIValue['LcBarcode'],$mappingOurBobBarcodeToDefacto[$tmpAPIValue['LcBarcode']],$mappingWaybillNumber[$tmpAPIValue['LcBarcode']]);
//
//                                    file_put_contents('actionPrintingBoxLabel-re.csv',print_r("cccc-5",true)."\n"."\n",FILE_APPEND);
//                                    file_put_contents('actionPrintingBoxLabel-re.csv',print_r($mappingOurBobBarcodeToDefacto,true)."\n"."\n",FILE_APPEND);
//                                    file_put_contents('actionPrintingBoxLabel-re.csv',print_r($mappingWaybillNumber,true)."\n"."\n",FILE_APPEND);
                                }
                            }
//                            file_put_contents('actionPrintingBoxLabel-re.csv',print_r("cccc-6",true)."\n"."\n",FILE_APPEND);
                        }
//                        file_put_contents('actionPrintingBoxLabel-re.csv',print_r("cccc-7",true)."\n"."\n",FILE_APPEND);
                        foreach ($stocks as $keyStock=>$stock) {
                            $stocks[$keyStock]['box_barcode'] = $mappingOurBobBarcodeToDefacto[$stock['box_barcode']];
                            $stocks[$keyStock]['our_box_barcode'] = $stock['box_barcode'];
                            $stocks[$keyStock]['waybill_number'] = $mappingWaybillNumber[$stock['box_barcode']];
...
//                        die("cccc-8");
//                        file_put_contents('actionPrintingBoxLabel-re.csv',print_r("cccc-8",true)."\n"."\n",FILE_APPEND);
//                        file_put_contents('actionPrintingBoxLabel-re.csv',print_r($stocks,true)."\n"."\n",FILE_APPEND);
                        $outboundPreparedDataRows = DeFactoSoapAPIV2Manager::preparedSendOutBoundFeedBackData($stocks,$outboundOrderItem);
                        if($outboundPreparedDataRows) {
                            foreach($outboundPreparedDataRows as $outboundPreparedDataRow){
...
//                    file_put_contents('actionPrintingBoxLabel-re.csv',print_r("cccc-9",true)."\n"."\n",FILE_APPEND);
//                    file_put_contents('actionPrintingBoxLabel-re.csv',print_r($rowsDataForAPI,true)."\n"."\n",FILE_APPEND);
//                    die("cccc-9");
                    if (!empty($rowsDataForAPI)) {
//                        $api = new DeFactoSoapAPIV2Manager();
//                        $api->SendOutBoundFeedBackData($rowsDataForAPI);
                        file_put_contents('SendOutBoundFeedBackDataSerialize-re.csv',serialize($rowsDataForAPI)."\n",FILE_APPEND);
                    }
                }
                $outboundOrderModel->status = Stock::STATUS_OUTBOUND_PREPARED_DATA_FOR_API;
                $outboundOrderModel->save(false);
...
//        die('ok');
    ...
    * */
    public function actionValidatePrintingBoxLabel()
    {
        ...
        $runNext = "no";
        $message = "Вы отсканировали меньше 90% заказа, вы уверены, что хотиете распечатать этикетки?";
        ...
        if($oo = OutboundOrder::find()->andWhere(['id'=>$outboundOrderIDs])->one()) {
            if($oo->client_id == Client::CLIENT_DEFACTO) {

                if($this->isRusBelStore($oo->to_point_id) && $this->isNotEmptyKgM3ForRusBel($oo->id)) {
                    return [
                        'runNext'=>$runNext,
                        'message'=>'Это заказ для Рус/Бел в котором не заполнены размер или вес',
                    ];
                }
                ...
                //BEGIN////
                if($this->isEmptyM3Box($oo)) {
                    return [
                        'runNext'=>'no',
                        'message'=>'В заказе есть короба без указанного размера',
                    ];
                }
                //END////
    ...
    private function isEmptyM3Box($outboundOrderID) {
        $outboundService = new \common\modules\outbound\service\Service();
        return $outboundService->isExistEmptyM3($outboundOrderID);
    }

    private function isRusBelStore($toStoreID) {
        $storeService = new \common\modules\store\service\Service();
        return $storeService->isRusStore($toStoreID) || $storeService->isBelStore($toStoreID);
    }

    private function isNotEmptyKgM3ForRusBel($outboundOrderID) {
        $outboundService = new \common\modules\outbound\service\Service();
        return $outboundService->isExistEmptyKg($outboundOrderID) || $outboundService->isExistEmptyM3($outboundOrderID);
    }
...
//    }
    ...
    public function actionListForPrintBoxLabel() {
        $searchModel = new OutboundOrderGridSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        ...
        return $this->render('print-box-label-list', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
            'clientsArray' => $clientsArray,
     ...
     * @param string $plids Picking list IDs
     * */
    public function actionPrintingBoxLabelIfError()
    { // printing-box-label-if-error
        $id = Yii::$app->request->get('id');

        $qIDs = OutboundPickingLists::find()
            ->select('id')
...
//                        die;
    ...
    public function printPikingList($items) {
//        <?php
/**
 * Created by PhpStorm.
 * User: kitavrus
...
//                    '      <th width="25%" align="center" valign="middle" border="1"><strong>' . Yii::t('outbound/forms', 'Product Model') . '</strong></th>' .
//                    '   </tr>';
...
//                $pdf->writeHTMLCell(110, 0, 10, 6, 'Лист на сборку № <span style="font-size: 5mm; font-weight: bold; ">' . $orderNumber . "</span> " . " [ " . $parentOrderNumber . " ] ".'<br /><span style="font-size: 5mm; font-weight: bold; ">' . $clientTitle .' / ' . $shopToTitle . ' </span>' , 0, 0, false, true, 'L');
                ...
                if ( !($opl = OutboundPickingLists::findOne(['barcode' => trim($PickingListBarcode)])) ) {
                    ...
                    $opl->client_id = $outboundModel->client_id;
                    $opl->status = OutboundPickingLists::STATUS_END;
                    ...
                    OutboundOrder::updateAll(['status'=>Stock::STATUS_OUTBOUND_PICKED, 'cargo_status'=>OutboundOrder::CARGO_STATUS_IN_PROCESSING],['id'=>$orderID]);
                    OutboundOrderItem::updateAll(['status'=>Stock::STATUS_OUTBOUND_PICKED],['outbound_order_id'=>$orderID]);
                    $newRecordFlag = true;
                }
