...
use stockDepartment\modules\wms\models\ReturnFormNew;
...
//                if(!$apiResponse['response']->GetKoliResult->IslemBasarili && $byProduct != 1) {
                    ...
                    $model->addError('returnform-order_number','Это не возвратный короб');
                }
            } else {
    ...
    * */
    public function actionPrintBoxBarcode()
    {
        ...
        $byProduct = Yii::$app->request->post('by-product');
        $boxBarcode = Yii::$app->request->post('box-barcode');
...
//                    if(isset($apiResponse['response']->KoliIadeKabulResult)) {
//                        $responseAPIMessage = $apiResponse['response']->KoliIadeKabulResult->Message;
//                        $responseAPIMessageCode = $apiResponse['response']->KoliIadeKabulResult->MessageCode;
//                        if (isset($apiResponse['response']->KoliIadeKabulResult->Koli)) {
//                            $koliResponse = $apiResponse['response']->KoliIadeKabulResult->Koli;
//                        }
//                    }

                    if(isset($apiResponse['response']->IadeKabulResult)) {
                        $responseAPIMessage = $apiResponse['response']->IadeKabulResult->Message;
                        $responseAPIMessageCode = $apiResponse['response']->IadeKabulResult->MessageCode;
                        if(isset($apiResponse['response']->IadeKabulResult->Koli)) {
...
/*                else {
                ...
                if(!empty($koliResponse)) {
                    ...
                    $koliResponse = $extraFields['koliResponse'];
                    $koliResponseData['BelgeID'] = isset($koliResponse['BelgeID']) ? trim($koliResponse['BelgeID']) : '';
                    $koliResponseData['DepoID'] = isset($koliResponse['DepoID']) ? trim($koliResponse['DepoID']) : '';
                    $koliResponseData['IrsaliyeSeri'] = isset($koliResponse['IrsaliyeSeri']) ? trim($koliResponse['IrsaliyeSeri']) : '';
                    $koliResponseData['IrsaliyeNo'] = isset($koliResponse['IrsaliyeNo']) ? trim($koliResponse['IrsaliyeNo']) : '';
...
//            die;
            ...
            if((isset($koliResponseData['KoliBarkod']) && !empty($koliResponseData['KoliBarkod']))) {
                ...
                $extraFields['responseAPIMessage'] = $responseAPIMessage;
                $extraFields['responseAPIMessageCode'] = $responseAPIMessageCode;
...
//                $errors[] = ['returnformnew-order_number'=> [Yii::t('return/messages', 'Api DeFacto не ответила, сообщите об этом Азамату, Абзалу, Игорю')."<br />".' DE FACTO API MESSAGE '.$m]];
                $errors= ['returnformnew-order_number'=> [Yii::t('return/messages', 'Api DeFacto не ответила, сообщите об этом Азамату, Абзалу, Игорю')."<br />".' DE FACTO API MESSAGE '.$m]];
                return [
                    'success'=>'0',
                    'errors'=>$errors,
            ...
            //return $this->redirect(['/returnOrder/default/index']);
//            $errors[] = ['returnformnew-order_number'=> [Yii::t('return/messages', 'Для этой накладной уже напечатали этикетку или вы не выбрали накладную')]];
            $errors = ['returnformnew-order_number'=> [Yii::t('return/messages', 'Для этой накладной уже напечатали этикетку или вы не выбрали накладную')]];
            return [
                'success'=>'0',
                'errors'=>$errors,
        ...
        $dataReturn =  $this->renderPartial('print/_box-label-pdf', ['model' => $model,'koliResponseData'=>$koliResponseData,'boxBarcode'=>$boxBarcode]);
        return Json::decode($dataReturn);
//        return $this->render('print/_box-label-pdf', ['model' => $model,'koliResponseData'=>$koliResponseData]);
    }
