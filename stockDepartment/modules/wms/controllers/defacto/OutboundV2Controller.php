...
class OutboundV2Controller extends Controller
{
    /*
    ...
    {
//        $oo = OutboundOrder::findOne(13850); // ошибка
//        $oo = OutboundOrder::findOne(13843); // все ок
//        $oo = OutboundOrder::findOne(13844); // все ок
//        $isRusBel = $this->isRusBelStore($oo->to_point_id);
//        $isEmptyKgM3ForRusBel = $this->isNotEmptyKgM3ForRusBel($oo->id);
//
//        echo "<br />";
//        echo "<br />";
//        echo "<br />";
...
//        if($isRusBel && $isEmptyKgM3ForRusBel ) {
//            echo ("EMPTY");
//            return [
//                'runNext'=>'no',
...
//        }
//
//        VarDumper::dump($isRusBel,10,true);
//        VarDumper::dump($isEmptyKgM3ForRusBel,10,true);
//        die("OK");
...
//        return $this->renderAjax('_sub-order-grid', [
    ...
    * */
    public function actionPrintPickList()
    {
        ...
        $items = [];
        if (is_array($ids)) {
            $items = OutboundOrder::find()->andWhere(['id' => $ids])->asArray()->all();
        }
    ...
    public function actionPickingListBarcodeScanningHandler()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            if($oo = $opl->outboundOrder){
				...
				$storeName = $oo->toPoint->getPointTitleByPattern();
            }
...
/*
			...
			$stockIdIfIsBoxOnlyOur = null;
            if(BarcodeManager::isBoxOnlyOur($model->product_barcode)) {
                $productStockBarcodeX = Stock::find()->select('product_barcode,id')->andWhere([
                    'status' => [
                        Stock::STATUS_OUTBOUND_PICKED,
                        Stock::STATUS_OUTBOUND_SCANNING
                    ],
				...
				if($productStockBarcodeX){
                    $model->product_barcode = $productStockBarcodeX->product_barcode;
					$stockIdIfIsBoxOnlyOur = $productStockBarcodeX->id;
                };
            }
...
//                    'outbound_picking_list_id' => $qIDs,
            ...
            if ($stock = Stock::find()->where([
			 ...
			 ->andFilterWhere(['id'=>$stockIdIfIsBoxOnlyOur])
			->one()) {
...
//            die($modelDP);
            ...
            if($outboundOrderModel->client_id == Client::CLIENT_DEFACTO && YII_ENV == 'prod' && $outboundOrderModel->id != 39125 ) { // id = 2 Дефакто
...
//                    StockExtraField::deleteAllById(Stock::find()->select('id')->andWhere(['outbound_order_id'=>$outboundOrderModel->id])->column());
                    file_put_contents('actionPrintingBoxLabel.csv',print_r("START ",true).date("H:i:s")."\n"."\n",FILE_APPEND);
//                    file_put_contents('actionPrintingBoxLabel.csv',print_r($outboundOrderItems,true)."\n"."\n",FILE_APPEND);
                    foreach ($outboundOrderItems as $outboundOrderItem) {
						...
						file_put_contents('actionPrintingBoxLabel.csv',date("H:i:s")."\n",FILE_APPEND);
...
//                                    }
//                                    die("cccc-5");
                                    file_put_contents('actionPrintingBoxLabel.csv',print_r("cccc-5 ".date("H:i:s"),true)."\n"."\n",FILE_APPEND);
									file_put_contents('actionPrintingBoxLabel.csv',date("H:i:s")."\n",FILE_APPEND);
...
//                            die("cccc-6");
                            file_put_contents('actionPrintingBoxLabel.csv',print_r("cccc-6 ".date("H:i:s"),true)."\n"."\n",FILE_APPEND);
                        }
						
						
//                        die("cccc-7");
                        file_put_contents('actionPrintingBoxLabel.csv',print_r("cccc-7 ".date("H:i:s"),true)."\n"."\n",FILE_APPEND);
						
						
						// Swap return LC barcode Start ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
						$returnBoxes = Stock::find()->select('box_barcode, inbound_client_box')
											->andWhere(['outbound_order_item_id' => $outboundOrderItem->id,"is_product_type"=>Stock::IS_PRODUCT_TYPE_RETURN]) // ,'status'=>[Stock::STATUS_OUTBOUND_PRINTING_BOX_LABEL, Stock::STATUS_OUTBOUND_SCANNED]
											->andWhere(['status' => [
								Stock::STATUS_OUTBOUND_SCANNED,
								Stock::STATUS_OUTBOUND_PRINT_BOX_LABEL,
								Stock::STATUS_OUTBOUND_PRINTING_BOX_LABEL,
							...
							->asArray()
							->all();
						// Swap return LC barcode End ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
						
						
                        foreach ($stocks as $keyStock=>$stock) {
							
							// Swap return LC barcode Start ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
							$mappingWithReturn = $mappingOurBobBarcodeToDefacto[$stock['box_barcode']];
							foreach ( $returnBoxes as $keyRStock => $returnStock) {
								if ($stock['box_barcode'] == $returnStock["box_barcode"]) {
									$mappingWithReturn = $returnStock["inbound_client_box"];
								}
							}

							//$stocks[$keyStock]['box_barcode'] = $mappingWithReturn;
							// Swap return LC barcode End ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
//                        die("cccc-8");
                        file_put_contents('actionPrintingBoxLabel.csv',print_r("cccc-8 ".date("H:i:s"),true)."\n"."\n",FILE_APPEND);
                        file_put_contents('actionPrintingBoxLabel.csv',print_r($stocks,true)."\n"."\n",FILE_APPEND);
                        ...
                        file_put_contents('actionPrintingBoxLabel.csv',print_r("cccc-8-1 ".date("H:i:s"),true)."\n"."\n",FILE_APPEND);
                    }
                    file_put_contents('actionPrintingBoxLabel.csv',print_r("cccc-9 ".date("H:i:s"),true)."\n"."\n",FILE_APPEND);
                    file_put_contents('actionPrintingBoxLabel.csv',print_r($rowsDataForAPI,true)."\n"."\n",FILE_APPEND);
//                    die("cccc-9");
                    if (!empty($rowsDataForAPI)) {
						 file_put_contents('actionPrintingBoxLabel.csv',print_r("DeFactoSoapAPIV2Manager Start ".date("H:i:s"),true)."\n"."\n",FILE_APPEND);
                        
						$outboundOrderModel->api_send_data = serialize($rowsDataForAPI);
						$outboundOrderModel->save(false);
...
//                        die;
						...
						file_put_contents('actionPrintingBoxLabel.csv',print_r("DeFactoSoapAPIV2Manager End ".date("H:i:s"),true)."\n"."\n",FILE_APPEND);
...
//        die('ok');
    ...
    * */
    public function actionValidatePrintingBoxLabel()
    {
        ...
        $message = "Вы отсканировали меньше 95% заказа, вы уверены, что хотиете распечатать этикетки?";
...
//    }
	...
	public function actionResendApi($orderId= null)
	{
//		die("actionResendOutboundOrder - DIE");
		...
		foreach ( $dataForToResendOutboundOrderAPI['OutBoundFeedBackThreePLResponse'] as $k => $item) {
			$LcBarcode =  (new OutboundBoxService())->getClientBoxByBarcode($item["LcBarcode"]);
			if (!empty($LcBarcode)) {
				$dataForToResendOutboundOrderAPI['OutBoundFeedBackThreePLResponse'][$k]["LcBarcode"] = $LcBarcode;
			}
		}
		//VarDumper::dump($dataForToResendOutboundOrderAPI,10,true);
		//die;
...
//		VarDumper::dump($res, 10, true);
		if (!$res['HasError']) {
			...
			Yii::$app->session->setFlash('success', $res['ErrorMessage']);
		}
    ...
    public function actionResendApi_TO_DELETE()
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
...
//                        die;
	...
	/*
	* Print pick list excel
	*
	* */
	public function actionPrintPickListExl()
	{
		$idsData = Yii::$app->request->get('ids');
		...
		$i = 1;
		$activeSheet->setCellValue('A' . $i, Yii::t('outbound/forms', 'Secondary address')); // +
		$activeSheet->setCellValue('B' . $i, Yii::t('outbound/forms', 'Primary address')); // +
		$activeSheet->setCellValue('C' . $i, Yii::t('outbound/forms', 'Product Barcode')); // +
		$activeSheet->setCellValue('D' . $i, Yii::t('outbound/forms', 'Кол-во')); // +
		$activeSheet->setCellValue('E' . $i, Yii::t('outbound/forms', 'Product Model')); // +

		if (empty($items)) {
			return "";
		}
		
		$parentOrderNumber = $items[0]['parent_order_number'];
		...
		foreach($itemsProcessQuery->batch($batchCount) as $values) {
			...
			foreach($values as $value) {
				$i++;
				$sa = explode('-', trim($value['secondary_address']));
				...
				$secondaryAddress = $stage."-".$row."-".$rack."-".$level;

				$activeSheet->setCellValue('A' . $i,$secondaryAddress); // +
				$activeSheet->setCellValue('B' . $i, $value['primary_address']); // +
				$activeSheet->setCellValue('C' . $i, $value['product_barcode']); // +
				$activeSheet->setCellValue('D' . $i, $value['items']); // +
				$activeSheet->setCellValue('E' . $i,$value['product_model']); // +
		...
		header('Content-Disposition: attachment;filename="pick-list-' .$parentOrderNumber . '.xlsx"');
