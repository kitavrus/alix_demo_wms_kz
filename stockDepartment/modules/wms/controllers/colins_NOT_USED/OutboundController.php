...
//        if ($order = InboundOrder::find()->andWhere(['client_box_barcode' => $model->box_barcode, 'client_id' => $model->client_id])->one()) {
        if ($order = InboundOrder::find()->andWhere(['client_box_barcode' => $boxBarcode, 'client_id' => $model->client_id])->one()) {
    ...
    public function actionPrintSortingListByBox()
    {
        ...
        if ($model->load(Yii::$app->request->get())) {
            ...
            if ($order = InboundOrder::find()->andWhere(['client_box_barcode' => $boxBarcode, 'client_id' => $client_id])->one()) {
                //Если уже распределение на короб распечататли
                if($order->status == Stock::STATUS_INBOUND_SORTED) {

                   $stockAll =  Stock::find()->select('outbound_order_id,id,product_barcode,product_model, count(id) as alloc_qty')->andWhere(['inbound_order_id'=>$order->id])->asArray()->groupBy('outbound_order_id, product_barcode')->all();

                    if(!empty($stockAll)) {
                        foreach($stockAll as $stock) {
                            if($oo =  OutboundOrder::findOne($stock['outbound_order_id'])) {
                                $outputData[$stock['product_barcode']][] = [
                                    'outbound_order_id' => $oo->id,
                                    'shop_id' => $oo->to_point_id,
                                    'product_barcode' => $stock['product_barcode'],
                                    'product_model' => $stock['product_model'],
                                    'expected_qty' => $stock['alloc_qty'],
                                ];
                            } else {
                                $remnantInBox[ $stock['product_barcode']] = $stock['alloc_qty'];
                            }
                        }
                    }
                    if($this->printType == 'html'){
                        Yii::$app->layout = 'print-html';
                        return $this->render('/print/print-allocate-list-html', [
                        ...
                        return $this->render('print/print-allocate-list', [
//                    return $this->renderPartial('print-allocate-list-html', [
                            'outputData' => $outputData,
                            'orderNumber' => $model->box_barcode,
                            'storeArray' => $storeArray,
                            'remnantInBox' => $remnantInBox,
...
//            if ($order = InboundOrder::find()->andWhere(['client_box_barcode' => $model->box_barcode, 'client_id' => $client_id])->one()) {
                ...
                $outboundIds = OutboundOrder::find()
                                ->select('id')
                                ->where(['parent_order_number' => $order->parent_order_number,'client_id' => $client_id, 'status' => [
                                                                            Stock::STATUS_OUTBOUND_NEW,
                                                                            Stock::STATUS_OUTBOUND_SCANNING,
...
//        $storeArray = TLHelper::getStockPointArray($client_id, true, false, '{shop_code}');
    ...
    public function actionScanningForm()
    {
        $client_id = Client::CLIENT_COLINS;
        $storeArray = TLHelper::getStockPointArray($client_id, true, false, '{internal_code}');
        $orders = OutboundOrder::find()
                    ->where(['client_id' => $client_id, 'status' => [Stock::STATUS_OUTBOUND_NEW, Stock::STATUS_OUTBOUND_SCANNING,Stock::STATUS_OUTBOUND_PICKED]])
                    ->orderBy('to_point_id')
                    ->all();
...
//            $stockArrayByPL = OutboundOrder::getItemsById($model->order_shop, true);
        ...
        return [
            ...
            'allocated_qty' => isset($orderData['allocated_qty']) ? $orderData['allocated_qty'] : '0',
 ...
 * */
    public function actionProductBarcodeScanningHandler()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            $stock = $model->findStockForScanning();
            $allocatedStockItem = $model->findReservedItem();

            if(!$stock && $allocatedStockItem){
                $model->createStockItemForScanning();
                $stock = $model->findStockForScanning();
            }
            //Ищем запись в Stock
            if ($stock) {
...
//                    if ($ioi->accepted_qty == $ioi->expected_qty || $ioi->accepted_qty == $ioi->allocated_qty) {
                    if ( $ioi->accepted_qty == $ioi->allocated_qty) {
...
//                    if ($outboundOrder->accepted_qty == $outboundOrder->expected_qty || $outboundOrder->expected_qty == $outboundOrder->allocated_qty) {
                    if ($outboundOrder->accepted_qty == $outboundOrder->allocated_qty) {
                ...
                $model->addError('product_barcode', 'Товар не найден на стоке');
            }
...
//                $stock->status = Stock::STATUS_OUTBOUND_PICKED;
                ...
                if ($ioi = OutboundOrderItem::findOne(['product_barcode' => $model->product_barcode, 'outbound_order_id' => $stock->outbound_order_id])) {
                    ...
                    $ioi->accepted_qty = $countStockForOrderItem;
//                    $ioi->status = Stock::STATUS_INBOUND_SORTING;
                    $ioi->status = Stock::STATUS_OUTBOUND_SCANNING;
                    $ioi->save(false);
                }
...
//                $oo->status = Stock::STATUS_OUTBOUND_SCANNING;
                ...
                if ($inboundItem = InboundOrderItem::find()->where([
                    ...
                    // TODO тут у нас есть уже принятые товары (переделать)!!!!
...
//                        $ioi->status = Stock::STATUS_INBOUND_SORTING;
                        ...
                        $oo->accepted_qty = $countStockForOrder;
//                        $oo->status = Stock::STATUS_INBOUND_SORTING; // TODO ?? Какой тут лучше поставить статус
                        $oo->status = Stock::STATUS_OUTBOUND_SCANNING; // TODO ?? Какой тут лучше поставить статус
                        $oo->save(false);
...
//            'exp_qty' => isset($orderData['allocated_qty']) ? $orderData['allocated_qty'] : '0',
    ...
    * */
    public function actionPrintingDifferencesList()
    {
        ...
        if($this->printType == 'html'){
            Yii::$app->layout = 'print-html';
            return $this->render('print/printing-differences-list-html', ['items' => $items,'order_shop'=>$order_shop]);
        } else {
            return $this->render('print/printing-differences-list-pdf', ['items' => $items,'order_shop'=>$order_shop]);
        }
    }
    ...
    public function actionPrintingBoxContent()
    {
       // $items = [];
        ...
        $stockItems = Stock::find()
            ...
            ->asArray()
            ->all();
        //VarDumper::dump($stockItems, 10, true); die;
//        if ($stockItems) {
//            foreach ($stockItems as $stockItem) {
//                if ($pb = ProductBarcodes::find()->andWhere(['client_id' => $client_id, 'barcode' => $stockItem['product_barcode']])->one()) {
//                    if ($product = $pb->product) {
//                    $items[$stockItem['box_barcode']]['products'][] = [
//                        'product_barcode' => $stockItem['product_barcode'],
//                        'model' => $product->model,
//                        'color' => $product->color,
//                        'size' => $product->size,
//                        'qty' => $stockItem['product_qty'],
//                    ];
//
//                }
//                }
//            }
//        }
        if($outboundOrder = OutboundOrder::findOne($outboundOrderId)){
        ...
        if($this->printType == 'html'){
            Yii::$app->layout = 'print-html';
            return $this->render('_box-content-html', [
                'items' => $stockItems,
                'box_barcode' => $box_barcode,
                'toPoint' => $toPoint,
                'clientID' => $client_id,
