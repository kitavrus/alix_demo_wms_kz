...
use stockDepartment\modules\wms\managers\erenRetail\checkBox\forms\CheckBoxForm;
use stockDepartment\modules\wms\managers\erenRetail\checkBox\repository\CheckBoxSearch;
use stockDepartment\modules\wms\managers\erenRetail\checkBox\repository\CheckBoxRepository;
...
use stockDepartment\modules\wms\managers\erenRetail\checkBox\entities\CheckBox;
...
//        $checkBoxForm->title = date('d-m-Y');
    ...
    public function actionExportToExcelWithProducts($showDiff = null) {
        ...
        foreach ($dps as $checkBox) {
            ...
            $checkBoxList = CheckBoxStock::find()->andWhere(['check_box_id'=>$checkBox['id']])->orderBy('status')->all();
            foreach ($checkBoxList as $productRow) {

                if(!is_null($showDiff)) {
                    if($productRow['status'] == \common\b2b\domains\checkBox\constants\CheckBoxStatus::END_SCANNED) {
                        continue;
                    }
                }
                ...
                $activeSheet->setCellValue('F' . $i, $productRow['stock_id']);
            }
            $i++;
        }
 ...
 public function actionExportToExcelWithProductsRow($showDiff = null) {
        ...
        $activeSheet->setCellValue('E' . $i, 'status')->getColumnDimension('E')->setAutoSize(true); // +
        $activeSheet->setCellValue('F' . $i, 'skuId')->getColumnDimension('F')->setAutoSize(true); // +
        $activeSheet->setCellValue('G' . $i, 'stockId')->getColumnDimension('G')->setAutoSize(true); // +
        ...
        $service = new CheckBoxService();
        $allInventoryKeyList = $service->getAllInventoryKeyList();
        $stockRowAllBoxes = $service->getAllBoxesFromStockByAddress($searchModel->place_address);
		$minus = 0;
			foreach ($dps as $checkBox) {
				$checkBoxList = CheckBoxStock::find()->andWhere(['check_box_id'=>$checkBox['id']])->orderBy('status')->all();

				foreach ($checkBoxList as $productRow) {
					if (isset($stockRowAllBoxes[$productRow['box_barcode']])) {
						unset($stockRowAllBoxes[$productRow['box_barcode']]);
					}
					...
					$activeSheet->setCellValue('G' . $i, $productRow['stock_id']);
					
				}
				if (count($checkBoxList) != $minus) {
					//$i++;
				}
				$minus = 0;
			}

		foreach ($stockRowAllBoxes as $box=>$data) {

			$activeSheet->setCellValue('A' . $i, \yii\helpers\ArrayHelper::getValue($allInventoryKeyList,$productRow['inventory_id']));
			$activeSheet->setCellValue('B' . $i, $box);
			$activeSheet->setCellValue('C' . $i, $data["secondary_address"]);
