...
use common\clientObject\hyundaiAuto\outbound\forms\BeginEndPickListForm;
use common\clientObject\hyundaiAuto\outbound\forms\OutboundForm;
use common\clientObject\hyundaiAuto\outbound\service\OutboundService ;
use common\clientObject\hyundaiAuto\outbound\service\OutboundReservationService;
use Yii;
...
//            $expectedQty = intval($orderInfo->order->expected_qty);
    ...
    /*
     * Scanning form handler Is Product Qty
     * */
    public function actionProductQtyHandler()
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
        ...
        $scanForm = new OutboundForm();
        $scanForm->scenario = 'onProductQty';
        if ($scanForm->load(Yii::$app->request->post()) && $scanForm->validate()) {
            $service = new OutboundService($scanForm->getDTO());
