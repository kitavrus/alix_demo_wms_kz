...
use stockDepartment\modules\wms\models\InboundForm;
use Yii;
...
class InboundController extends Controller
{
    ...
    public function actionGetScannedProductById()
    {
        ...
        if( $io = InboundOrder::findOne($id)) {
            $items = $io->getOrderItems()->select('*,(expected_qty - accepted_qty) as order_by')->orderBy(new Expression('order_by!=0 DESC'))->asArray()->all();
...
//                                 'status'=>[Stock::STATUS_INBOUND_SCANNED,Stock::STATUS_INBOUND_OVER_SCANNED]]);
  ...
  * @return JSON true or errors array
     * TODO NOT USED
  * */
    public function actionUploadFromApi_NOT_USED_TO_REMOVE()
    { // upload-from-api
