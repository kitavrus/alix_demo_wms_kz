...
class ColinsInboundForm extends Model {
 ...
 * */
    ...
    {
        $value = $this->$attribute;
        $inboundOrderIds = $this->getItemsByParentOrder();

        if(!self::checkProductBarcode($value, $inboundOrderIds)) {
    ...
    public function validateProductInBox($attribute, $params)
    {
        ...
        $inboundOrderIds = $this->getItemsByParentOrder();
        if(!self::checkProductInBox($value,$box_barcode,$inboundOrderIds)) {
...
//            }
//        }
//        if(InboundOrder::find()->where(['status'=>Stock::STATUS_INBOUND_COMPLETE,'id'=> $inboundOrderIds])->exists()) {
...
//        $value = $this->$attribute;
        ...
        if(!Stock::find()->where(['inbound_order_id'=>$inboundOrderIds,'primary_address'=>$box_barcode,'status'=>[Stock::STATUS_INBOUND_SCANNED,Stock::STATUS_INBOUND_OVER_SCANNED]])->exists()) {
            $this->addError($attribute, '<b> [ ' . $box_barcode . ' ] </b> '.Yii::t('inbound/errors','Box is empty')); // Этого товара нет в укзанном коробе
        }
...
//        }
    ...
    {
        return [
            [['client_id','party_number'], 'required'],
            [['client_id'], 'integer'],
            [['order_number','box_barcode','product_barcode','party_number'], 'string'],
            [['box_barcode'], 'validateBoxBarcode'],

            [['client_id','party_number'], 'required','on'=>'ConfirmOrder'],
            [['client_id','party_number','box_barcode'], 'required','on'=>'ClearBox'],
            [['box_barcode'], 'validateClearBox','on'=>'ClearBox'],
            [['client_id','party_number','box_barcode','product_barcode'], 'required','on'=>'ClearProductInBox'],
    ...
    public function checkProductInBox($productBarcode,$box_barcode,$inbound_ids)
    {
        return Stock::find()->where(['inbound_order_id'=>$inbound_ids,'primary_address'=>$box_barcode,'product_barcode'=>$productBarcode,'status'=>[Stock::STATUS_INBOUND_SCANNED,Stock::STATUS_INBOUND_OVER_SCANNED]])->exists();
//        return InboundOrderItemProcess::find()->where(['box_barcode'=>$box_barcode,'product_barcode'=>$productBarcode])->exists();
    }

    public function getItemsByParentOrder(){
        return InboundOrder::find()->select('id')
            ->andWhere(['consignment_inbound_order_id'=>$this->party_number, 'client_id' => $this->client_id])
            ->asArray()
            ->column();
    }
