...
class OutboundCrossDockForm extends Model
{
    ...
    public $box_size_code; // Размер короба
    ...
    * */
    ...
    {
        return [
            [['internal_barcode', 'box_barcode','box_size_code'], 'string'],
            [['internal_barcode', 'box_barcode','box_size_code'], 'trim'],
            ...
            [['box_size_code','box_barcode','to_point','internal_barcode'], 'required','on'=>'vScanningBoxSizeCode'],
        ];
    }
...
* */
    public function vScanningBoxSizeCode($attribute, $params)
    {
        $value = $this->$attribute;
        if(!BarcodeManager::isProduct($value) && !BarcodeManager::isM3BoxBorder($value)) {
            $this->addError($attribute, '<b>['.$value.']</b> '.Yii::t('outbound/errors','Вы ввели неправильный размер короба'));
        }
    }
