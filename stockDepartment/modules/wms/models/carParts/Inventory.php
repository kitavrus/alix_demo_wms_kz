...
namespace stockDepartment\modules\wms\models\carParts;
 ...
 */
class Inventory extends \common\modules\stock\models\Inventory
{
    ...
    const INVENTORY_FILE_NAME_ERROR = 'inventory20170105.csv';
...
//            return "№ ".$data->order_number.' - '.$data->client->title;
    ...
    **/
    ...
    {
        return Stock::find()->where(['status_inventory'=>self::STATUS_SCAN_YES,'secondary_address'=>$minMax,'primary_address'=>$boxBarcode,'inventory_id'=>$inventory_id])->count();
    }
    ...
    * */
    ...
    {
        return Stock::find()->where(['status_inventory'=>self::STATUS_SCAN_YES,'secondary_address'=>$minMax,'inventory_id'=>$inventory_id])->count();
    }
...
//    }
    ...
    public static function saveAcceptedQtyByRow($qty,$row,$inventory_id)
    {
        ...
        $levelNumber = Inventory::getLevelNumber($row);
       if($ir = InventoryRows::find()->andWhere(['column_number'=>$columnNumber,'floor_number'=>$floorNumber,'level_number'=>$levelNumber,'inventory_id'=>$inventory_id])->one()) {
           $ir->accepted_qty = $qty;
           $ir->save(false);
       }
