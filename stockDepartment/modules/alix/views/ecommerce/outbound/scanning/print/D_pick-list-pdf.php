...
foreach ($items as $order) {
    ...
    $itemsProcessQuery = Stock::find()
        ->select('GROUP_CONCAT(id) as ids, product_barcode, primary_address, secondary_address, product_model, product_name, field_extra2, count(*) as items ')
        ->andWhere([
...
//                'status' => Stock::STATUS_OUTBOUND_RESERVED,
    ...
    if ($count = $itemsProcessQuery->count()) {
        ...
        foreach ($itemsProcessQuery->batch($batchCount) as $values) {
            ...
            $pdf->writeHTMLCell(110, 0, 10, 6, 'Лист на сборку № <span style="font-size: 3mm; font-weight: bold; ">' . $orderNumber . '</span><br /><span style="font-size: 3mm; font-weight: bold; ">' . $clientTitle . ' / ' . $shopToTitle . ' </span>', 0, 0, false, true, 'L');
            ...
            foreach ($values as $value) {
                ...
                $value['product_name'] = OutboundOrderItem::find()->select('product_name')->andWhere(['product_model'=>$pbr])->scalar();
