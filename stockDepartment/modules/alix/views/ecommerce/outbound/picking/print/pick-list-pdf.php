...
foreach ($outboundList as $outboundId => $productsInOrder) {
    ...
    $clientTitle = "Alix Avien";// $productsInOrder[0]['order']['clientShipmentSource'];
    ...
    $structure_table = '';
    if ($productsInOrder) {
        $headerText = '<span style="font-size: 4mm; font-weight: bold; ">Лист на сборку № <span style="font-size: 3mm; font-weight: bold; ">' . $orderNumber . '</span><br /><span style="font-size: 3mm; font-weight: bold; ">' . $clientTitle. ' </span><br /><span style="font-size: 3mm;">' . $currentDateTime->format("d-m-Y H:i") . ' </span>';
...
//        $pdf->write1DBarcode($orderID.'-'.$orderNumber, 'C128', '', '', 270, 20, 0.4, $style, 'M'); // T M B N
        $pdf->write1DBarcode($orderNumber, 'C128', '', '', 270, 20, 0.4, $style, 'M'); // T M B N
        $pdf->Ln(20);
        ...
        $structure_table = '<table width="100%" cellspacing="0" cellpadding="4" border="1">' .
            ...
            '      <th width="30%" align="center" valign="middle" border="1"><strong>' . Yii::t('outbound/forms', 'Название') . '</strong></th>' .
            '      <th width="15%" align="center" valign="middle" border="1"><strong>' . Yii::t('outbound/forms', 'Цвет/Бренд ') . '</strong></th>' .
            '   </tr>';
        ...
        foreach ($productsInOrder as $rowOnStock) {
                $structure_table .= '<tr align="center" valign="middle">' .
                    '<td align="center" valign="middle" border="1">' . $dash($rowOnStock['placeAddressBarcode']) . '</td>' .
                    '<td align="center" valign="middle" border="1">' . $dash($rowOnStock['boxAddressBarcode']) . '</td>' .
                    '<td align="left" valign="middle" border="1">' . $dash($rowOnStock['productBarcode']) . '</td>' .
                    '<td align="left" valign="middle" border="1">' . $dash($rowOnStock['productQty']) . '</td>' .
					'<td align="left" valign="middle" border="1">' . $dash($rowOnStock['productName']) . '</td>' .
					'<td align="left" valign="middle" border="1">' . $dash($rowOnStock['productColor']) . ' / ' . $dash($rowOnStock['productBrand']) . '</td>' .
					'</tr>';
        }
