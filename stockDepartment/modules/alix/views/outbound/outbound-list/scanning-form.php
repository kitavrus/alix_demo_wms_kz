...
?>
<h1>Лист отгрузки Intermode Ecommerce</h1>
...
<?= $form
    ...
    <?= $form
        ...
        ->dropDownList(\stockDepartment\modules\alix\controllers\outbound\domain\constants\CourierCompany::getAll(), [
    ...
    <div class="row" style="margin: 20px 1px">
        <?= Html::tag(
            'span',
            Yii::t('outbound/buttons', 'Содержимое листа'),
            ['data-url' => Url::toRoute('show-order-in-list'), 'class' => 'btn btn-success', 'id' => 'outboundlistform-show-order-in-list-bt', 'style' => 'margin-left:10px;']
        ) ?>
        <?= Html::tag(
            'span',
            Yii::t('outbound/buttons', 'Упакован но не отсканирован в лист отгрузки'),
            ['data-url' => Url::toRoute('show-packed-order-but-not-scanned-to-list'), 'class' => 'btn btn-success', 'id' => 'outboundlistform-packed-not-scanned-bt', 'style' => 'margin-left:10px;']
        ) ?>
        <?= Html::tag(
            'span',
...
<script src="/js/e-commerce/defacto/outbound-list-form.js"></script>