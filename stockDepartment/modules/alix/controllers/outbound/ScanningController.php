...
use stockDepartment\modules\intermode\controllers\outbound\domain\ScanningForm;
use Yii;
...
use stockDepartment\modules\intermode\controllers\stock\domain\StockService;
...
//			$productID = $productService->getProductIdByBarcode($model->product_barcode);
		//	$productGuid = $productService->getGuidIdByBarcode($model->product_barcode);
			$productGuid = $productService->getLastGuidIdByBarcode($model->product_barcode);
...
//					$productGuid = $productService->getGuidIdByBarcode($item['product_barcode']);
					$productGuid = $productService->getLastGuidIdByBarcode($item['product_barcode']);
					$ioi = OutboundOrderItem::findOne(['product_sku' =>$productGuid, 'outbound_order_id' => $item['outbound_order_id']]);
					if ($ioi) {
...
//        }
    ...
    public function actionPrintingBoxLabel()
    {
        ...
        if (isset($items[0]['outbound_order_id'])) {
			...
			$cms = new CronManagerService();
			$cms->makeJobB2BOutbound($outboundOrderModel->id,$outboundOrderModel->order_number);
        }
