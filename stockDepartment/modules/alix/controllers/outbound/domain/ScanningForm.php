...
//    }
    ...
    * */
	...
	public function validateBoxBarcodeOnly4000($attribute, $params){
		...
		$opl = OutboundPickingLists::findOne(['barcode' => $this->picking_list_barcode]);
		if ($opl) {
			$isExist =  Stock::find()
						   ->andWhere(['box_barcode'=>$boxBarcode])
						   ->andWhere('outbound_order_id != :outbound_order_id',[':outbound_order_id'=>$opl->outbound_order_id])->exists();
			if($isExist) {
				$this->addError($attribute, '<b>['.$boxBarcode.']</b> '.Yii::t('inbound/errors','В этом коробе есть товары из другого заказа'));
			}
		}
	}
}