...
//			die;
			...
			foreach ($dataForFindList as $prodctBercode=>$itemData) {
				...
				$totalDiffQty += $itemData["diff_qty"];
			...
			foreach ($dataForFindList as $prodctBercode=>$itemData) {
				//$productTotal = 0;
				foreach ($itemData["items"] as $item) {
					for ($i = 1; $i <= $itemData["expected_qty"]; $i++) {
						$row = [
							$orderNumber,
							$item["box_barcode"],
							$item["product_barcode"],
//							$item["qty"],
			...
			foreach ($dataForFindList as $prodctBercode=>$itemData) {
				foreach ($itemData["items"] as $item) {
					$findBoxUnique[$item["box_barcode"]] = $item["box_barcode"];
				}
			}

			$contentFindBox = "Номер накладной;Короб;"."\n";
			foreach ($findBoxUnique as $box=>$item) {
						$row = [
							$orderNumber,
							$box,
						];
					$contentFindBox .= implode($row, $delimiter) . "\n";
			}

			if ($totalDiffQty > 0) {
				$filename = $orderNumber . "-validate.csv";
				return Yii::$app->response->sendContentAsFile($contentValidate, $filename);
			}

			//$filename = $orderNumber . "-find.csv";
			//return Yii::$app->response->sendContentAsFile($contentFindBox, $filename);
	...
	public function actionIndex2()
	{
		...
		if ($model->load(Yii::$app->request->post()) && $model->validate()) {
			...
			foreach ($orders as $productBarcode => $qty) {
				$productOnStock = Stock::find()
									   ->select("`box_barcode`, `product_barcode`")
										// ->select("`box_barcode`, `product_barcode`, COUNT(`product_barcode`) as qty")
														   ->andWhere([
											"outbound_order_id" => $orderId,
											"product_barcode" => $productBarcode,
...
//			VarDumper::dump($productOnStock,10,true);
//			die;
				$qtyTotal = $qty;
				if (!empty($productOnStock)) {
					foreach ($productOnStock as $item) {
						$dataForFindList[$item["product_barcode"]][$item["box_barcode"]][] = [
							"box_barcode"=>$item["box_barcode"],
							"product_barcode"=>$item["product_barcode"],
						];
					}
			...
			$delimiter = ";";
			$findBoxUnique = [];
			foreach ($dataForFindList as $productBarcode=>$boxes) {
				foreach ($boxes as $box=>$item) {
					$findBoxUnique[$box] =$box;
				}
			}
...
//			echo "<br />";
//			echo "<br />";
//			VarDumper::dump($findBoxUnique,10,true);
			$filename = $orderNumber . "-upload.csv";
			return Yii::$app->response->sendContentAsFile($contentUpload, $filename);
			die;
		}
