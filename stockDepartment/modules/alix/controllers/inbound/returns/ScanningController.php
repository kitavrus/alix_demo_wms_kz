...
namespace stockDepartment\modules\alix\controllers\inbound\returns;
...
use stockDepartment\modules\alix\controllers\inbound\returns\domain\InboundRepository;
use stockDepartment\modules\alix\controllers\inbound\returns\domain\InboundForm;
...
class ScanningController extends Controller
{
            ...
            if ($productsInBox) {
                foreach ($productsInBox as $item) {
                    ...
                    if ($ioi) {
                        ...
                        $countStockForItem = EcommerceStock::find()
                            ->andWhere(
                                [
                                    'inbound_id' => $model->order_number,
                                    'product_barcode' => $item->product_barcode,
                ...
                $countStockForOrder = EcommerceStock::find()
                    ->andWhere(
                        [
                            'inbound_id' => $model->order_number,
                            'status' => EcommerceStock::STATUS_INBOUND_SCANNED,
    ...
    public function actionScanProductInBox()
    {
        ...
        $isFirstScan = empty($io->begin_datetime);

        if ($isFirstScan) {
            try {
                (new InboundScanningService())->sendStatusInWork($orderNumber);
            } catch (\Throwable $e) {
                Yii::error('sendStatusInWork failed: ' . $e->getMessage(), __METHOD__);
            }
            $io->begin_datetime = time();
        }

        $io->expected_qty += 1;
        $io->accepted_qty += 1;
        $io->status = EcommerceStock::STATUS_INBOUND_SCANNING;
        $io->save(false);

        $stock = new EcommerceStock();
        $stock->client_id = $clientId;
        $stock->box_address_barcode = $boxBarcode;
        $stock->inbound_id = $orderNumber;
        $stock->inbound_item_id = $ioi->id;
