...
namespace stockDepartment\modules\alix\controllers\ecommerce\outbound\domain\entities;
...
class EcommerceOutboundSearch extends EcommerceOutbound
{
    ...
    public function search($params)
    {
        ...
        $query->andFilterWhere([
            'ecommerce_outbound.id' => $this->id,
            'ecommerce_outbound.status' => $this->status,
        ...
        if (!empty($this->productBarcode)) {
        	$products = explode(",",$this->productBarcode);
        ...
        if (!empty($this->outboundBoxBarcode)) {
            $subQuery = Stock::find()
                ...
                ->andWhere(['like', 'box_barcode', $this->outboundBoxBarcode]);
            $query->andWhere(['id' => $subQuery]);
        }
        ...
        if (!empty($this->findType))
        {
            switch($this->findType) { // 5 4 3
                ...
                case '3': // есть расхождения между reserved scanned
                    $query->andWhere('expected_qty != accepted_qty');
                    $query->andWhere('status != :status',[':status'=>OutboundStatus::getCANCEL()]);
                break;
