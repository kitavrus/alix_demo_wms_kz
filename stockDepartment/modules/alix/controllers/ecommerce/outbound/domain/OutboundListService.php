...
use app\modules\ecommerce\controllers\alix\outbound\domain\constants\StockOutboundStatus;
...
class OutboundListService
{
    ...
    public function scanPackageBarcode($dto) {
    	...
    	//VarDumper::dump($this->getOrderIdByPackageBarcode($dto->barcode));
    	//die;
        $orderInfo = $this->outboundService->getOrderInfo($this->getOrderIdByPackageBarcode($dto->barcode));
    ...
    public function printList($dto) {
        ...
        foreach($outboundList as $listRow) {
            ...
            $this->acceptedOrder($listRow->our_outbound_id);
        }
	...
	{
		//One function
		$orderInfo =$this->outboundService->getOrderInfo($orderId);
		...
		$stocks = EcommerceStock::find()->andWhere(["client_id" => 103, 'outbound_id' => $orderInfo->order->id])->all();
    ...
    public function packedOrderButNotScannedToListOrder() {
        return EcommerceOutbound::find()->select('id, status, order_number, packing_date, client_ReferenceNumber, client_CargoCompany')
            ->andWhere([
                'client_id'=>103,
                'status'=>OutboundStatus::getPACKED(),
            ])
...
//                'courier_company'=>$courierCompany,
    ...
    public function isOrderPackaged($placeBarcode)
    {
		...
		$orderId = Stock::find()
			->select('ecom_outbound_id')
			->andWhere([
				'box_barcode' =>$placeBarcode,
				'client_id' => 103,
			])->scalar();
        if(empty($orderId)) {
            return false;
       ...
       $orderInfo = $this->outboundService->getOrderInfo($this->getOrderIdByPackageBarcode($dto->barcode));
        ...
        switch($dto->courierCompany) {
            case CourierCompany::LAMODA :
                $result = $orderInfo && $orderInfo->order->client_ShipmentSource == $dto->courierCompany;
            break;
...
//                $result = $orderInfo && $orderInfo->order->client_ShipmentSource == KaspiDefaultValue::CLIENT_CARGO_COMPANY;
//            break;
        }
	...
	public function getOrderIdByPackageBarcode($packageBarcode) {
		return Stock::find()->select('ecom_outbound_id')->andWhere([
			'box_barcode' =>$packageBarcode,
			'client_id' => 103
		])->scalar();
	}

    public function getEcommerceOutbound($orderNumber)
    {
