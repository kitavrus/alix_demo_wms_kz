...
namespace stockDepartment\modules\alix\controllers\ecommerce\outbound\domain;

use stockDepartment\modules\alix\controllers\ecommerce\outbound\domain\constants\OutboundStatus;
use stockDepartment\modules\alix\controllers\ecommerce\outbound\domain\mapper\OutboundAPIMapper;
...
class OutboundPickingService
{
	...
	public function __construct()
	{
		...
		//$this->apiService = new \stockDepartment\modules\alix\controllers\ecommerce\outbound\domain\mock\OutboundAPIService();
		$this->apiService = new \stockDepartment\modules\alix\controllers\ecommerce\outbound\domain\OutboundAPIService();
	}
	/**
	 * @param void
...
//					'clientShipmentSource' => $order->client_ShipmentSource,
			...
			$stockInfo = $this->repository->getEcommerceStockByOutboundAndBarcode($productOnStock['ecom_outbound_id'],$productOnStock['product_barcode']);
			...
			$orderListForSort [] = [
				...
				// primary_address = place_address_barcode (полка), secondary_address = box_address_barcode (короб).
				// См. алиасы в OutboundRepository::getStockBeforePrintPickingList.
				'placeAddressBarcode'=>$productOnStock['primary_address'],
				'productBarcode'=>$productOnStock['product_barcode'],
				'productId'=>$productOnStock['product_id'],
				'boxAddressBarcode'=>$productOnStock['secondary_address'],
				'productModel'=>$itemInfo->product_model,
				'productName'=> $stockInfo ? $stockInfo->product_name : ($itemInfo ? $itemInfo->product_name : ''),
				'productColor'=> $itemInfo->product_color,
				'productBrand'=> $itemInfo->product_brand,
