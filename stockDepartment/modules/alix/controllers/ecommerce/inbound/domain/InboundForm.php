...
class InboundForm extends Model
{
    ...
    public function validateBoxBarcode($attribute, $params)
    {
        ...
        $count = EcommerceStock::find()
            ...
            ->andWhere('inbound_id != :inbound_id', [':inbound_id' => $inbound_order_id])
            ->exists();
        ...
        $count = EcommerceStock::find()
            ...
            ->andWhere('inbound_id != :inbound_id AND place_address_barcode != ""', [':inbound_id' => $inbound_order_id])
            ->exists();
    ...
    public function validateProductInBox($attribute, $params)
    {
        ...
        if (EcommerceInbound::find()->andWhere(['status' => EcommerceStock::STATUS_INBOUND_COMPLETE, 'id' => $orderNumberId])->exists()) {
    ...
    public function validateClearBox($attribute, $params)
    {
        ...
        if (EcommerceInbound::find()->andWhere(['status' => EcommerceStock::STATUS_INBOUND_COMPLETE, 'id' => $this->order_number])->exists()) {
    ...
    public function checkProductBarcode($productBarcode, $inbound_order_id)
    {
        return EcommerceInboundItem::find()
            ->andWhere(['product_barcode' => $productBarcode, 'inbound_id' => $inbound_order_id])
            ->exists();
    }
    ...
    public function setScannedStatus()
    {
        ...
        if ($stock) {
            ...
            $inboundItemOne = EcommerceInboundItem::find()
                ->andWhere([
                    'inbound_id' => $this->order_number,
                    'product_barcode' => $this->product_barcode,
                ])
            ...
            if (!$inboundItemOne) {
                $inboundItemOne = new EcommerceInboundItem();
                $inboundItemOne->inbound_id = $this->order_number;
                $inboundItemOne->product_barcode = $this->product_barcode;
                $inboundItemOne->product_accepted_qty = 1;
