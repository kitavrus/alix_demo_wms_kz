...
use app\modules\ecommerce\controllers\alix\outbound\domain\mapper\OutboundAPIMapper;
use stockDepartment\modules\alix\controllers\api\v1\outbound\service\OutboundService;
use Yii;
...
class OutboundController extends Controller
{
    ...
    public function actionOrders()
    {
		...
		$dto = $is->requestToCreateDTO($request);
		file_put_contents("OutboundController_actionOrders_".$dto->orderNumber.".log", date(DATE_ISO8601)."\n"."request:"."\n".print_r($dto,true)."\n"."\n",FILE_APPEND);
		...
		foreach ($dto->items as $item) {
			...
			file_put_contents("Outbound_b2b_".$dto->orderNumber.".csv", implode(";",$row)."\n",FILE_APPEND);
		}

		$order = $is->addOrder($dto);
	...
	private function notifyByTg($orderId) {
		...
		TelegramalixB2BNotification::sendMessageIfNewOutboundOrder(
...
//		$content .= 'FULL'.";";
		...
		$searchModel = new \stockDepartment\modules\alix\controllers\api\v1\outbound\repository\OutboundOrderGridSearch();
