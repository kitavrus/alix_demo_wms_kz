...
//                'xxxxx'=> $countStockForItem,
     ...
     * */
    public function actionGetListDifferences()
    {
...
//                ->orderBy(" CASE WHEN expected_qty < accepted_qty
//THEN expected_qty > accepted_qty
//ELSE expected_qty END ASC")
                ->asArray()
                ->all();
        }
        if($this->printType == 'html'){
            Yii::$app->layout = 'print-html';
            return $this->render('list-differences-html',['items'=>$items]);
        }
        return $this->render('list-differences-pdf',['items'=>$items]);
    }
...
//                $inbound->status = InboundOrder::STATUS_COMPLETE_BY_API;
    ...
    public function actionPrintUnallocatedList()
    {
        ...
        if($this->printType == 'html'){
            Yii::$app->layout = 'print-html';
            return $this->render('print-unallocated-box-html',['items'=>$items]);
        }
        return $this->render('print-unallocated-box-pdf',['items'=>$items]);
    }
}